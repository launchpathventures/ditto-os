...
**Greeter representative contract** (Brief 259, Insight-226): On public reputation-bearing surfaces such as `/people/[handle]`, Alex/Mira speak as REPRESENTATIVES, never IMPERSONATORS. They talk about the absent user in third person, cite only public/on-request facts, honor anti-persona rules silently, and gate intro requests through self-contained `AuthorizationRequestBlock` artifacts. This is the public-profile inversion of process-os Ask Charlie's "you ARE Charlie" pattern.
...
**Connecting a project is a process, not a metadata operation** (Briefs 224-226, absorbing Insight-205): When a user pastes a GitHub URL the resulting `projects` row enters `status='analysing'` and triggers a `project-onboarding.yaml` system process. That process clones the repo (depth=1, single-branch via `isomorphic-git`), runs read-only detector handlers (build system, test framework, CI provider, existing AI/agent harness markers), scores persona-fit to a user-facing descriptor, matches against a Researcher-curated landscape corpus, recommends a runner + trust tier, and surfaces an `AnalyserReportBlock` for the user to review. The user picks a runner + tier and the atomic three-write commit (insert `project_runners`, generate-and-hash bearer, flip `status='active'`) graduates the project. The harness extends INTO every repo Ditto manages — connecting a project is the first sample of the process Ditto will run inside it.
...
**Multi-step pipeline output passing — handlers query `step_runs.outputs` explicitly.** The harness pipeline does NOT auto-merge previous step outputs into the next step's `inputs`. Each step's handler receives `processRun.inputs` (the original kickoff inputs) plus the harness-injected `_stepRunId` + `_processRunId` — nothing more. When a downstream step needs data produced by an earlier step (Brief 226 analyser handlers reading the cloned-repo `tempDir`; Brief 228 retrofitter's `dispatch-write` reading the plan from `generate-plan`; the cron-driven outbound monitor reading sibling-step outputs), the handler queries `step_runs.outputs` from the database keyed by `processRunId` + the upstream step's `stepId`. The canonical helper is `readPriorStepOutputs(processRunId, db)` exported from `@ditto/core` at `packages/core/src/harness/step-output-reader.ts` — it returns `Record<stepId, outputs>` with last-write-wins semantics for retried steps (Brief 228 §AC #3, Insight-217 absorption). Architects writing multi-step process YAMLs must explicitly plan how downstream steps read upstream output; loose phrases like "passes via handler context" are misleading. Defensive fallbacks (logged inline recompute when an upstream step's output is missing) are valuable — silent recovery would hide upstream-step failures.
...
**Workspace Local Bridge** (Brief 212): A harness extension that lets cloud-hosted Ditto dispatch shell commands to the user's laptop / Mac mini via an outbound-dial WebSocket daemon (`ditto-bridge`). The cloud side is a `bridge.dispatch` built-in tool + a `dispatchBridgeJob` handler that traverses the standard trust-gate / Insight-180 stepRunId guard before any DB write, so every dispatch produces one `harness_decisions` row keyed on stepRunId. State machine and wire types live in `@ditto/core/bridge`; daemon ships as `packages/bridge-cli/`. Pairing is a code-then-JWT bootstrap (15-min TTL, single-use, bcrypt-hashed). The bridge is a transport — no agent code runs on user hardware. See Brief 212 for full ACs (queue persistence on offline, mid-job disconnect resume, 60s heartbeat, 10-min orphan detection, 4-tier trust semantics enforced by the existing `trust-gate.ts`).

**Deploy Gate** (Brief 220, sub-brief of 214): A production-deploy boundary integrated into the work-item state machine. The `briefState` enum (Brief 223 origin) is extended with `deploying`, `deployed`, `deploy_failed`, slotted between `shipped` and `archived`. Transitions are validated by `transitionBriefState()` in `@ditto/core/work-items/state-machine.ts` (engine-core pure module mirroring the runner-dispatch state machine). The handler `processProductionDeployStatus()` in `cloud-runner-fallback.ts` consumes `deployment_status` GitHub webhook events filtered to the per-project `deployTargetEnvironment` (default `Production`) and drives `shipped → deploying → deployed | deploy_failed`. Out-of-order webhook delivery is admitted as direct `shipped → deployed` and `shipped → deploy_failed` transitions (the `deploying` intermediate is informational, not load-bearing). Idempotent retries are rejected by the state machine and audited with `metadata.transitionRejected = true`. Insight-180 bounded-waiver pattern applies (the GitHub webhook has no `stepRunId` — every activity row writes `metadata.guardWaived = true`). The `queued` event additionally emits a `mobileApproveAction` payload that the conversation surface renders as a one-tap GitHub-Mobile deep-link `ActionBlock`. The deploy gate itself is GitHub-side (a `production` Environment with a Required Reviewer); Ditto is the observer + UX surface only — no Ditto-side approval mechanism duplicates the GitHub gate.
...
**Design-token-vs-component-class layering** (Brief 230): The styling layer for Layer 6 splits cleanly in two. **Tokens** (colors, fonts, type scale, spacing, radius, shadows) live inside the Tailwind v4 `@theme` block in `packages/web/app/globals.css` — generated as utility classes (`bg-vivid`, `text-text-primary`) and consumable via `var(--color-*)` from any CSS context. **Component classes** (the six block primitives `.block.breakdown` / `.decision` / `.plan` / `.compare` / `.evidence` / `.people` plus chrome `.block-head` / `.bbody` / `.block-foot` / `.bkind` / `.bopen` and the composition helper `.alex-line`) live in a sibling `packages/web/app/design-system.css` imported into `globals.css`. The design package preserved at `.context/attachments/design-package-2026-04-27/` is the source-of-truth-mirror — the canonical class names from `workspace/blocks.js` are honoured verbatim. Block renderers compose by adding bundled class strings (`<section className="block findings tone-positive">`) instead of authoring per-renderer Tailwind utility maps. Brief 230 also introduced one Ditto-original primitive — **`.block.findings`** with tone modifiers (`tone-positive` / `tone-caution` / `tone-negative` / `tone-info` / `tone-vivid`) — a sectioned-list-with-tone-coded-header that the design package's `.block.evidence` (kv-pair) shape doesn't fit; it's the home for AnalyserReportBlock's Strengths / Watch-outs / Missing surfaces. Per-renderer promotion (utility-mapped → bundled-class composition) happens organically as briefs touch each renderer; mass promotion is intentionally deferred.
...
Everything the user sees flows through **ContentBlocks** — typed, structured data units defined in `packages/core/src/content-blocks.ts` (re-exported from `src/engine/content-blocks.ts`) and rendered by the block registry (`packages/web/components/blocks/block-registry.tsx`). **28 ContentBlock types** form a discriminated union with compile-time exhaustiveness checking. See ADR-021 addendums for the lineage; the engine source is authoritative.
...
| **Block renderers** | Block → AI Element mapping | `packages/web/components/blocks/` — 28 renderers |
