...
11. [ ] **Optional SSE event-stream observability works when configured.** Integration test with `observe_events=true`: the adapter opens an SSE subscription post-dispatch; allowlisted events (`agent.message`, `agent.tool_use`, `agent.tool_result`, `agent.error`) surface as activity-log entries on the work item. SSE failures (mocked connection drop, mocked 5xx) downgrade silently to polling-only without failing the dispatch. SSE closes on terminal state OR after `sse_max_duration_ms`.
