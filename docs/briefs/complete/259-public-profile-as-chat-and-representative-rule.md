...
**Date:** 2026-05-12 · doc-amended 2026-05-13 for Insight 234 pivot
**Status:** built (pending merge — code/migrations/tests live in `launchpathventures/hotfix-network-users-handle-migration` stash + untracked files; Insight 234 pivot from in-memory SSE bridge to durable outbox + pull-and-ack is now reflected in this brief)
**Depends on:**
...
7. `docs/insights/234-cross-deployment-inbox-delivery-needs-durable-pull-ack.md` — authored during this brief's build. Cross-deployment delivery must be a durable outbox on the sender side + pull-and-ack on the consumer side; in-memory SSE is a UI courtesy only, never the delivery contract.
...
- **`forward_note_to_user(stepRunId, userId, fromVisitor, factQuestionMd)` is a side-effecting self-tool.** Persists to a new `networkForwardedNotes` pgTable on the Network tier + cross-deployment delivers via the durable `networkWorkspaceDeliveries` outbox + pull-and-ack pattern (Insight 234): the Network calls `queueWorkspaceInboxDelivery({ userId, kind: "forwarded_note", blocks, stepRunId })` which writes a delivery row and emits an SSE nudge for live UI invalidation; the workspace polls `/api/v1/workspace/inbox/import` on inbox view + on SSE, imports each delivery into local `activities`, and ACKs back. Persistence is the contract; SSE is the courtesy. Refuses without `stepRunId` outside `DITTO_TEST_MODE` per Insight-180. Tool name string registered in `src/engine/tool-resolver.ts` `builtInTools` MUST match the directive reference in the visitor-lane Greeter directive — a unit test asserts both name strings are equal (catches drift like `forward_note_to_user` vs `forwardNoteToUser`).
- **Cross-deployment auth contract (Insight-231 + Insight-234).** The intro request that originates from `ditto.partners/people/{handle}` and lands in the user's workspace inbox at `ditto-ws-{handle}.up.railway.app` MUST be valid in the consuming deployment AND must arrive via a durable delivery, not a live-only event:
  - **Sender:** Network Service writes the artifact to the durable `networkWorkspaceDeliveries` outbox via `queueWorkspaceInboxDelivery({ userId, kind: "visitor_intro_request", blocks, dedupeKey, stepRunId })` and emits an SSE nudge for live UI invalidation. Per Insight 234, durability is the contract; SSE is the courtesy.
  - **Target deployment:** the user's workspace (looked up via the existing Network ↔ workspace token; the workspace's `DITTO_NETWORK_URL` + `DITTO_NETWORK_TOKEN` env vars identify the user-id binding).
  ...
  - **Consumer:** the workspace's `/api/v1/workspace/inbox/import` endpoint authenticates with its `DITTO_NETWORK_TOKEN`, GETs pending deliveries from `/api/v1/network/workspace-deliveries`, inserts each into the local `activities` table idempotent on `(action="workspace_inbox_delivery", entityType="network_workspace_delivery", entityId=<delivery.id>)`, and POSTs the imported ids back to ACK. **ACK retry is part of idempotency (Insight 234 implication 3):** an already-imported row still includes its delivery id in the next ACK batch — local-dedupe alone is not enough or the Network queue can remain pending forever.
  - **Terminal-state persistence:** the workspace's `/api/v1/workspace/inbox/authorization` route writes approve/reject decisions to local state so a reload does not resurrect a stale `pending` (Insight 234 implication 4).
...
| `src/engine/forward-note-to-user.ts` | Create: `forwardNoteToUser({ stepRunId, userId, fromVisitor, factQuestionMd })`. Persists to `networkForwardedNotes`; delegates cross-deployment delivery to `queueWorkspaceInboxDelivery({ userId, kind: "forwarded_note", blocks, stepRunId })` (durable outbox + SSE nudge per Insight 234). Refuses without `stepRunId` outside `DITTO_TEST_MODE` (Insight-180). |
| `src/engine/forward-note-to-user.test.ts` | Create: tests for `stepRunId` enforcement, persistence shape, cross-deployment inbox-drop via the durable outbox. |
| `src/engine/workspace-inbox-delivery.ts` | Create: `queueWorkspaceInboxDelivery({ userId, kind, blocks, dedupeKey, stepRunId })` writes to the durable outbox and emits an SSE nudge. `listPendingWorkspaceInboxDeliveries({ userId, limit })` + `markWorkspaceInboxDeliveriesImported({ userId, ids })` form the pull-and-ack pair consumed by `/api/v1/network/workspace-deliveries`. Per Insight 234. |
...
| `packages/core/src/db/network/schema.ts` | Modify: add `networkForwardedNotes` pgTable (`{ id (uuid), userId → networkUsers, fromVisitorName?, fromVisitorOrg?, factQuestionMd, visitorIp, visitorSessionId, createdAt, status: "pending" \| "answered" \| "dismissed" }`, index on `userId`) AND `networkWorkspaceDeliveries` pgTable (`{ id (uuid), userId → networkUsers, kind: NetworkWorkspaceDeliveryKind, blocks: ContentBlock[], dedupeKey?, sourceStepRunId?, status: "pending" \| "imported" \| "failed", createdAt, updatedAt }`, index on `(userId, status)`). Durable-outbox row per Insight 234. |
| `packages/core/src/content-blocks.ts` | Modify: add `variant?: "hero-primary" \| "hero-secondary" \| "body"` to `TextBlock` for Greeter-introduction typography on the visitor surface. |
| `drizzle/network/0003_create_network_forwarded_notes.sql` | Create via `drizzle-kit generate` against the network config. |
| `drizzle/network/0004_create_network_workspace_deliveries.sql` | Create via `drizzle-kit generate` against the network config (durable cross-deployment outbox; Insight 234). |
| `drizzle/network/meta/_journal.json` | Append BOTH new entries for migrations 0003 + 0004 (Insight-190: verify SQL file exists for every journal entry, idx values sequential). |
| `packages/web/app/api/v1/network/workspace-deliveries/route.ts` | Create: Network-side outbox endpoints. `GET` returns pending deliveries for the authenticated workspace user (Bearer-token auth via `authenticateRequest`); `POST` ACKs imported delivery ids and marks them `status="imported"`. |
| `packages/web/app/api/v1/workspace/inbox/import/route.ts` | Create: workspace-side puller (cookie-auth). Calls the Network outbox GET; inserts each delivery into local `activities` table idempotent on `(action, entityType, entityId)`; POSTs the imported ids back to ACK. ACK retry includes already-imported ids (Insight 234 implication 3). |
| `packages/web/app/api/v1/workspace/inbox/import/route.test.ts` | Create: tests for under-limit pull, idempotent re-import, ACK retry behavior on already-imported rows. |
| `packages/web/app/api/v1/workspace/inbox/authorization/route.ts` | Create: workspace-side approve/reject handler for imported `AuthorizationRequestBlock` rows. Persists terminal state locally so reloads do not resurrect a stale `pending` (Insight 234 implication 4). |
| `packages/web/app/api/v1/workspace/inbox/authorization/route.test.ts` | Create: tests for approve/reject persistence + reload-doesn't-resurrect-pending. |
| `packages/web/hooks/use-network-push.ts` | Modify: on `workspace_blocks_push` SSE event for `viewSlug: "inbox"`, trigger `POST /api/v1/workspace/inbox/import` to pull + import + ACK. SSE is the courtesy nudge; the import endpoint is the durability seam. |
| `packages/web/components/layout/views/inbox-view.tsx` | Modify: on mount, fire-and-forget `POST /api/v1/workspace/inbox/import` so pending deliveries that arrived while the user was offline import on first inbox view. |
...
14. [ ] BOTH new pgTables added to `packages/core/src/db/network/schema.ts`: `networkForwardedNotes` AND `networkWorkspaceDeliveries`. BOTH migration SQL files exist: `drizzle/network/0003_create_network_forwarded_notes.sql` and `drizzle/network/0004_create_network_workspace_deliveries.sql`. `drizzle/network/meta/_journal.json` has TWO new entries with sequential idx (Insight-190 dual-journal verification). The durable outbox (`networkWorkspaceDeliveries`) is the seam Brief 261 reuses for the introductions primitive.
...
## Reference Docs Updated (during design + build)
...
- `docs/insights/234-cross-deployment-inbox-delivery-needs-durable-pull-ack.md` — authored during this brief's build when the originally-specified "existing Network → workspace bridge" was discovered not to exist. The durable `networkWorkspaceDeliveries` outbox + pull-and-ack pattern replaces the in-memory-only `emitNetworkEvent()` for cross-deployment delivery; SSE remains as the live-UI nudge. Brief 261 (introductions) and any future Network → workspace artifact briefs MUST consume this seam — sender via `queueWorkspaceInboxDelivery`, consumer via `/api/v1/workspace/inbox/import`.
- `docs/architecture.md` — Layer 6 workspace / public profile section: `network_workspace_deliveries` is the seed durable-delivery primitive for Network → workspace Inbox artifacts.
- `docs/review-checklist.md` — added a cross-deployment delivery check distinct from the existing cross-deployment auth artifact check (per Insight 234 "Where It Should Land").
