...
**Date:** 2026-04-25 (amended 2026-04-27 post-build to reconcile drift items below)
**Status:** complete (2026-04-27 — post-Reviewer-fix; see `docs/state.md` Documenter checkpoint)

## Post-Build Amendments (2026-04-27 — drift reconciliation)

Builder + Reviewer flagged four reference-doc drift items via Insight-043. Amended in place rather than rolled into the Brief 214 phase doc (the items are Brief-216-specific shape questions, not phase-level).

1. **§D2 prompt-body field naming corrected.** The literal prompt body in §D2 used snake_case `runner_kind` + runner-state values (`succeeded|failed|cancelled`). The actual Brief 223 route schema (`workItemStatusUpdateSchema` in `@ditto/core/work-items`) accepts only camelCase + brief-state values (`backlog|approved|active|review|shipped|blocked|archived`). **Authoritative shape now**: the routine prompt instructs the session to POST `{ state: "review" | "blocked" | "archived", runnerKind: "claude-code-routine", externalRunId, stepRunId, prUrl?, error? }` mapped from outcome. The runner-state-shape Zod schema in `packages/core/src/runner/webhook-schema.ts` (`claudeCodeRoutineStatusPayload` / discriminated-union variant) remains as a parallel contract for future direct consumers; not on the route's hot path. State semantics: `review` = PR opened, ready for human; `blocked` = could not complete; `archived` = cancelled.

2. **§D10 `RunnerAdapter.execute()` signature corrected.** The brief's literal text said `execute(stepRunId, dispatchId, workItem, projectRunner)`. The actual interface in `packages/core/src/runner/interface.ts` is `execute(ctx: DispatchExecuteContext, workItem, project, projectRunner)` where `ctx` carries `{ stepRunId, processRunId, dispatchId, trust }`. **Authoritative signature**: the interface — not the brief's prose. The implementation in `src/adapters/claude-code-routine.ts` follows the interface.

3. **`ProjectRef` lacks `harnessType` — adapter does a runtime DB read with cache.** Brief D7 assumed the adapter could read `harnessType` from `ProjectRef` to choose catalyst-vs-native skill loading. The Brief 215 `ProjectRef` interface does not surface `harnessType`. **Authoritative pattern**: `defaultHarnessTypeFor(project, db)` in `src/adapters/claude-code-routine.ts` is async — checks the in-process cache (primed at boot via `primeHarnessTypeCacheFromDb`), falls back to a per-dispatch DB read on cache miss (with memoize), and `invalidateHarnessTypeCache(projectId)` is exported for callers that mutate `projects.harnessType` at runtime. Future: extend `ProjectRef` to carry `harnessType` in a follow-up to remove the DB roundtrip; tracked as a comment in `claude-code-routine.ts`.

4. **§D4 "Ditto's existing GitHub integration handler" was a forward reference, not a real existing component.** Brief D4 said the adapter's static `webhookSubscriptions` declaration would be "registered with Ditto's existing GitHub integration handler at `src/engine/integration-registry.ts`." That integration-registry is a YAML declaration loader, not a webhook event router; no GitHub webhook delivery infrastructure existed at brief-write time. **Authoritative shape**: a new HMAC-SHA256-verified receiver at `POST /api/v1/integrations/github/webhook` (Reviewer-fix commit `91f890f`) calls the kind-agnostic fallback handlers in `src/engine/github-events/cloud-runner-fallback.ts` directly. Configuration: `GITHUB_WEBHOOK_SECRET` env var (documented in `.env.example`); GitHub repo settings → Webhooks → events: `pull_request`, `workflow_run`, `deployment_status`. The static `webhookSubscriptions` declaration on the adapter is unused for now — kept as a TODO marker for future declarative subscription registration when multiple adapters subscribe to the same event with different match logic.

---

**Original status (pre-amendment):** draft
