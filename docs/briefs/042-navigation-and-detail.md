...
- **Capabilities:** Left sidebar navigation, process detail views (living roadmap + domain process), trust control surface, engine view (developer mode), three-panel layout, progressive reveal transition
...
| `packages/web/components/detail/process-detail.tsx` | Create: process detail container (routes to variant) |
...
- **Jobs affected:** Orient (sidebar status), Delegate (trust control), Decide (engine transparency)
- **Primitives involved:** Process Card (sidebar compact + detail expanded), Trust Control (natural language), Performance Sparkline, Plan/Task (living roadmap)
- **Process-owner perspective:** Rob clicks "Quoting" in sidebar → sees "How it works" (7 steps in plain English) + "How it's going" (34 quotes, 91% clean, sparkline trending up) + "How closely do you watch this?" slider. Jordan toggles Engine View → sees routing decisions, agent cost, memory assembly for each run.
- **Interaction states:**
...
- **Designer input:** UX spec sections 3.4-3.7 (sidebar, detail, trust, engine view), 5 (entry point logic)
...
5. [ ] Living roadmap variant: shows steps with ✓/●/○ icons, current step narration, mini activity feed
6. [ ] Domain process variant: shows "How it works" (plain language steps), "How it's going" (metrics + sparkline), recent runs
7. [ ] Trust control renders as natural language slider: "Check everything" ↔ "Let it run" with current position
8. [ ] Trust control shows evidence data as narrative (not metrics table): "31 approved without changes (91%)"
9. [ ] Trust control changes go through `trust-diff.ts` server-side, require user confirmation
10. [ ] Engine View toggle (hidden by default, enabled via settings or keyboard shortcut)
...
2. Review checks: user language throughout, trust control goes through engine, Engine View doesn't leak to non-dev users, progressive reveal works
