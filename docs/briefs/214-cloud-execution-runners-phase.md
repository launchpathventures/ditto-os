...
**Date:** 2026-04-25
**Status:** ready (2026-04-27 — user steer: drop Greptile/Argos entirely. Parent decomposition shrunk from 8 sub-briefs to 7; sub-brief 219 retired; default review path = `/dev-review` skill in Routine + Vercel preview URL inline card from `deployment_status` non-production events, both shipped by sub-brief 216, both already complete. The 2026-04-25 architect-checkpoint-#8 "Greptile/Argos optional integration" model is superseded by this revision.)
...
- **Greptile / Argos / any AI-review-or-visual-check SaaS integration — DROPPED entirely** (2026-04-27 user steer; supersedes 2026-04-25 "optional integration" framing). The default review path is `/dev-review` skill in-session inside the Routine + human review on PR (shipped by sub-brief 216). The default visual check is the Vercel preview URL inline card surfaced from `deployment_status` non-production events (also shipped by sub-brief 216). **No Greptile detector. No Argos detector. No `detectIntegrations()` function. No `project.has_greptile` / `project.has_argos` flags. No `approved-by-greptile` label automation. No `argos-ci` check-gate.** Users who already have Greptile/Argos installed on their repos will see those tools' GitHub-native PR comments + checks like any other GitHub user — Ditto neither detects nor adapts to them. If demand surfaces post-dogfood, a follow-on brief can re-add detection; nothing in the current substrate prevents future opt-in. Sub-brief 219 is RETIRED — see §Sub-brief Decomposition.
- **Auto-installing any third-party GitHub App on user repos.** Ditto only installs its own GitHub integration (Brief 200 / 218 patterns).
...
**D11: Default review + visual-check paths — Greptile/Argos DROPPED entirely (revised 2026-04-27 per user steer; supersedes 2026-04-25 "optional integration" model).**

- **Default review path (always on, ONLY path):** the `/dev-review` skill running inside the Routine session (or whichever runner dispatched the work) PLUS human review on the PR. No third-party SaaS required, ever. The Routine prompt is composed to invoke `/dev-review` after the implementation phase; its output appears as a PR comment authored by the routine's GitHub identity. Human review is the gate the work-item state machine waits on. **Already shipped by sub-brief 216** (`src/engine/runner-status-handlers/routine.ts` + the prompt composer at `src/adapters/claude-code-routine.ts`).
- **Default visual check (always on, ONLY path):** Vercel preview URL link surfaced in the work-item conversation surface as an inline card from the `deployment_status` event with `state: "success"` + non-production `environment`. Equivalent path works for Netlify / Cloudflare Pages / Render / Fly preview deployments — any non-production `deployment_status` event renders an inline preview-URL card. **Already shipped by sub-brief 216** (`cloud-runner-fallback.ts` + `routine-status-handler.ts`).
- **NO Greptile detector. NO Argos detector. NO `detectIntegrations()` function. NO `project.has_greptile` / `project.has_argos` flags. NO `approved-by-greptile` label automation. NO `argos-ci` check-gate.** Sub-brief 219 is RETIRED — see §Sub-brief Decomposition. If a user has Greptile or Argos installed on their repos, those tools' GitHub-native PR comments + checks appear like any other GitHub user — Ditto neither detects nor adapts.
- **Why dropped:** (a) the default `/dev-review` + human-PR + Vercel-preview path is already complete and works end-to-end on any repo; (b) detection complexity (per-project flags + daily cron + redetect button + conditional webhook handlers) was substantial overhead for marginal value; (c) users who actually want Greptile/Argos are paying for them already and see their output in the PR — Ditto doesn't need to gate on it; (d) reducing the scope frees sub-briefs 220 + 221 + 222 of any optional-integration coordination concerns.
- **Re-add path:** if post-dogfood demand surfaces, a future architect can write a fresh integrations-detection brief at any point; nothing in the current substrate prevents future opt-in. The cloud-runner-fallback.ts `check_run.completed` infrastructure that Brief 218 shipped (originally reserved for sub-brief 219) is generic enough to serve any future check-gate use case.
...
**Seven sub-briefs** along dependency seams (was 8 before the 2026-04-27 Greptile/Argos drop — sub-brief 219 retired). Bodies for sub-briefs 216-222 are deferred to subsequent Architect sessions per the parent-then-sub-brief sequence (Brief 209 precedent).
...
| ~~219~~ | ~~**Optional review/visual-check integrations**~~ | ~~review substrate~~ | **RETIRED 2026-04-27 per user steer.** Default `/dev-review` skill in Routine + human PR review (sub-brief 216) is the only review path; default Vercel preview URL inline card from `deployment_status` non-production events (sub-brief 216) is the only visual signal. No detector, no flags, no third-party SaaS adaptation. Re-add via fresh follow-on brief if post-dogfood demand justifies. | — |
...
| 222 | **End-to-end smoke tests** | proof | Playwright e2e suite covering: agent-crm cloud (Mac off, phone-only), agent-crm local (Mac on, runner forced local), native-Ditto cloud, native-Ditto local; runner-chain fail-over; runner_mode_required filter. Each scenario produces an artefact PR + `/dev-review` skill PR comment + Vercel preview URL inline card + deploy approval. | 8-10 |

**Sub-brief 215 is the critical path.** 216, 217, 218 can run in parallel after 215 merges. **220 (deploy-gate), 221 (mobile UX), and 222 (e2e smoke) can ship as soon as 216 merges** — the default review path (`/dev-review` inside the Routine + human review on PR) and the default visual signal (Vercel preview URL inline card) are both wired by 216. With sub-brief 219 retired, the dependency chain shortens: 220, 221, 222 have no remaining blockers post-216-merge.
...
| ~~`docs/briefs/219-greptile-argos-review.md`~~ | **RETIRED 2026-04-27** — sub-brief 219 dropped per user steer. No file to create. |
...
| ~~Greptile GitHub App~~ | greptile.com | **REJECTED 2026-04-27** | Sub-brief 219 retired per user steer; default review = `/dev-review` skill in Routine + human PR review (sub-brief 216). No third-party AI-review SaaS dependency. |
| ~~Argos GitHub App~~ | argos-ci.com | **REJECTED 2026-04-27** | Sub-brief 219 retired per user steer; default visual check = Vercel preview URL inline card from `deployment_status` non-production events (sub-brief 216). No third-party visual-diff SaaS dependency. |
...
| ~~`approved-by-greptile` label automation~~ | researcher report Gap 5 | **REJECTED 2026-04-27** | Sub-brief 219 retired; no Greptile detection ships. |

## User Experience

- **Jobs affected:** Delegate (primary — choosing where work runs), Decide (approving + selecting runner), Capture (runner-started/finished cards), Review (the `/dev-review` skill PR comment + the human PR review surface).
...
- **Interaction states:** loading (dispatching), dispatched (external link rendered), running (live external link with status), succeeded (PR opened, link to PR + `/dev-review` comment), failed/timed_out/rate_limited (retry button + auto-advance to next in chain), no-eligible-runner (clear "configure a runner" CTA → /projects/[slug]/runners), revoked (credential vault detected expired bearer; "re-pair routine" CTA).
...
11. [ ] ~~**Optional Greptile integration wires only when detected:**~~ **RETIRED 2026-04-27** — sub-brief 219 dropped per user steer; no Greptile detection ships.
12. [ ] ~~**Optional Argos integration wires only when detected:**~~ **RETIRED 2026-04-27** — sub-brief 219 dropped per user steer; no Argos detection ships.
13. [ ] ~~**Detection cron + manual redetect:**~~ **RETIRED 2026-04-27** — sub-brief 219 dropped per user steer; no `detectIntegrations()` cron ships.
...
**24 ACs — above the 8-17 band for a single build brief.** This is deliberate for a **parent brief**: these are phase-level gates spanning the (now 7) sub-briefs, not a single build session. Sub-briefs 215-218 + 220-222 each ship with 8-17 of their own ACs per `docs/dev-process.md:179-194` (brief sizing rule). ACs #11/#12/#13 are RETIRED 2026-04-27 alongside sub-brief 219; the effective AC count is 21 with 24 numbering preserved for git-blame stability.
...
10. ~~(Optional Greptile)~~ — Greptile detection retired 2026-04-27; if agent-crm has Greptile installed, its comment surfaces as a normal GitHub-native PR comment, no Ditto-side handling.
11. ~~(Optional Argos)~~ — Argos detection retired 2026-04-27; if agent-crm has Argos installed, its check appears as a normal GitHub-native check, no Ditto-side gating.
...
Work item against the Ditto repo itself: "Update copyright year in packages/web/app/layout.tsx footer." No Catalyst structures present.
...
# Smoke E — RETIRED 2026-04-27. Sub-brief 219 dropped per user steer. The default review + visual paths (smokes A-D) are the only paths.
```
...
- `docs/dictionary.md` post-build: Project, Runner, Runner Kind, Runner Mode, Runner Chain, Runner Dispatch, Mode-Required Constraint, Fallback Runner. (`Approved-by-Greptile Label` retired 2026-04-27 alongside sub-brief 219.)
