...
**Unlocks:**
- Future pricing/payments brief (post-30-day-validation per parent Deferred Items table). v1 free-only enforcement; downstream pricing should evaluate payment for successful outcomes, successful introductions, or repeated high-value connection workflows, not arbitrary paywalls.
...
| `packages/web/app/api/v1/network/workspace-provision/route.ts` | Create: self-service upsell provisioning route. Resolves the live expert/client lane session server-side, rejects any caller-supplied `stepRunId`, rate-limits by resolved user, then calls the existing workspace provisioner. This preserves the admin-only contract of `/api/v1/network/admin/provision` while allowing the chat CTA to work for normal users. |
...
| `packages/web/components/network/workspace-upsell-cta.tsx` | Create: chat-rendered CTA component with the verbatim copy + two buttons `[Yes, set up workspace] [Not now, just my card]`. Wires Yes button to `/api/v1/network/workspace-provision`, which wraps the existing workspace-provisioning flow without exposing the admin-only endpoint to the browser. |
...
- **Interaction states:**
  ...
  - **Workspace upsell shown:** CTA rendered in chat after the Q6 completion seam for the expert/client lane. User can dismiss without consequences.
...
14. [ ] Workspace upsell CTA renders with the parent's verbatim copy and `[Yes, set up workspace] [Not now, just my card]` buttons. The Yes button posts to `/api/v1/network/workspace-provision`, which resolves the live lane session and calls the existing provisioner server-side.
15. [ ] **HTTP route bypass-rejection (Insight-232 Implication 2) — BOTH intro-emission routes.** The visitor-lane route at `packages/web/app/api/v1/network/people/[id]/intro-request/route.ts` AND the client-lane route at `packages/web/app/api/v1/network/intros/route.ts` (or whatever concrete path Brief 261's build identifies for the client-lane intro emission) each reject any request body containing a `stepRunId` field, including falsy values, with a 4xx response — the audited-route wrapper is the only authority that may mint `stepRunId`, never the client. Verified by TWO route tests (one per route) that send `{ stepRunId: "fake" }` and falsy-field variants in the body and assert 4xx + `emit_intro_request` was NOT invoked + no `introductions` row written.
