...
1. **Runtime isolation for Network — parked with named triggers (updated 2026-04-19 after Browserbase peripheral research; applies Insight-200).** `BrowserRuntime` interface lands in 183; in-process local Playwright is v1. A hosted/sidecar/container implementation (Browserbase / Steel / Anchor / Hyperbrowser / Airtop, or a self-hosted Playwright sidecar) is **not** speculatively pre-briefed. Instead it is parked behind named triggers — any one of the following flips the decision to reopen:
   - **Trigger-A (multi-tenant Network deployment) — on-system signal:** `networkUsers` row count reaches ≥2 rows with `status = 'active'` against a single Network Service deployment, OR Phase 9 roadmap entry transitions to "multi-tenant-serving" status. Observable via `select count(*) from network_users where status = 'active'` + `docs/roadmap.md`. Boolean-testable.
   - **Trigger-B (anti-bot failure classifier) — gated on future classifier work:** *Inactive until Brief 184 ships.* When Brief 184 adds a session-expiry / anti-bot failure class to `classifyFailureType` (see `src/engine/heartbeat.ts:124` — current classes `confidence_low | timeout | dependency_blocked | external_error | max_retries | unknown` do **not** include anti-bot), this trigger activates as: ≥2 `browser.*` stepRuns for the same service classified `anti_bot` (or the equivalent new class name) within a 7-day window, queried from `step_runs` joined to `activities`. Until 184 lands, this trigger has no signal mechanism and is recorded as "pending classifier." PM MUST NOT declare it fired under the interim "human-inspects-traces" heuristic — subjective-feel triggers are the failure mode Insight-200 warns against.
   - **Trigger-C (compliance requirement) — off-system signal, explicitly acknowledged:** Any tenant contract requires HIPAA BAA, SOC 2 Type 2, or equivalent third-party-audited evidence for browser actions. This is contract-level, not database-level — surfaces via sales / onboarding conversation, not telemetry. On-system proxy (when tenant-record schema supports it in a later phase): `networkUsers.complianceTier = 'hipaa' | 'soc2'`. Until that column exists, Dev PM tracks this trigger as a manual flag on the onboarding conversation log.
   - **Trigger-D (sustained scale parallelism) — deferred pending observability:** The scale-parallelism signal (Chromium resource pressure on the hub) is the strongest theoretical reason to adopt a hosted runtime but Ditto has no current telemetry that attributes memory or CPU pressure to browser stepRuns specifically. Rather than name a trigger Ditto cannot observe, this trigger is **deferred until** a resource-attribution observability pass adds: (a) per-subsystem RSS / CPU samples on the heartbeat tick, AND (b) a concurrent-`browser.*`-stepRun counter. If that observability never ships, this trigger never activates and the scale-parallelism decision is instead driven by the commercial conversation with any future Browserbase Scale-tier counterparty — not by a silent subjective judgement.

   **Decision owner:** Dev PM (sequences the reopening against active phase). **Consulted:** Dev Architect (shapes the decision artefact at trigger-fire time). **Monitoring cadence:** Dev PM reviews the four triggers at every phase-boundary retrospective; missed-trigger-fires are a phase-close audit risk, surfaced in the retro artefact. **Decision artefact on trigger-fire:** a compact sub-brief under Brief 182's family implementing the chosen runtime as a `BrowserRuntime` implementation (e.g. a hypothetical `BrowserRuntimeBrowserbase`). **Brief number is claimed at trigger-fire, not reserved** — Insight-200 prohibits speculative number-claiming. **Research input already prepared:** `docs/research/browserbase-product-family.md` (2026-04-19) — factual inventory of Browserbase Cloud Runtime, Stagehand-on-Browserbase, Browse CLI, Director, plus Steel/Anchor/Hyperbrowser/Airtop as surfaced alternatives; pricing pinned at 2026-04-19 ($0/$20/$99/custom) but MUST be re-verified at trigger-fire. Interface shape in Brief 183 is the durable architectural commitment; named triggers are the reopening mechanism; this Open Question is the canonical home for the parking record.
