...
**Current phase:** Phase 10 **complete**. All 7 sub-briefs (039-046) shipped. 330 tests (23 test files). **Prototype-as-specification complete:** 28 prototypes (P00-P41) with 6 artifact viewers, full design system alignment. `.impeccable.md` is authoritative design spec. **ADR-024 (proposed):** Composable Workspace Architecture — three-tier model. **Brief 047 (draft):** Composition Engine. **Research (2026-03-28):** OpenOats meeting intelligence — pattern source for TS engine, **adopt** candidate for native macOS companion app. Three-phase flow: "Ditto Listens" (transcript→work items, 1-2 weeks) → "Ditto Speaks" (bidirectional knowledge surfacing, 1-2 weeks) → "Ditto Lives Here" (full macOS companion, 2-4 weeks). Report at `docs/research/openoats-meeting-intelligence.md`. Insight-106 captured. Landscape updated. Reviewed: PASS WITH FLAGS (both rounds, all flags resolved). Next: accept ADR-024, approve + build Brief 047. Architect to evaluate OpenOats fork if macOS companion is prioritised.
...
- **Prototype-as-Specification Strategy (2026-03-25 → 2026-03-27)** — Phase A structurally complete. **Prototype system overhaul executed (2026-03-27):** 28-screen navigation flow with visual vocabulary. Strategy doc at `docs/research/prototype-as-specification-strategy-ux.md`.
  ...
  - **P08 (Day Zero) simplified to v3** — single welcome page. No more tabs. Just "Hi. I'm Ditto" differentiation message + CTA into P08a. Connect/import removed from P08 tabs.
  - **P08a (Intake Conversation) restructured to 5 states:** (1) Opening — conversation + knowledge panel, (2) Synthesis — voice rules + "never do this" card, (3) Connect your world — connect services + import knowledge woven into conversation flow, (4) Process creation — guided hybrid process builder with steps/settings/provenance, (5) First output — UI mode-shifting layout transformation.
  - **Key design decisions:** Intake conversation IS the onboarding (not separate). Connect services and import knowledge happen conversationally during intake AND remain accessible from Settings/Knowledge Base permanently. Process creation happens AFTER connecting tools. Process creation is guided hybrid — collaborative definition, not accept/reject.
  - **Brand identity evolution (2026-03-26):** Major design session establishing Ditto's visual identity. Explored multiple directions (orange/ruby ribbon → pink/green gradient → blue/green organic → forest/emerald). Final palette: **Two-green system** — forest green (`#3D5A48`, brand anchor) + emerald (`#059669`, vivid accent). Cool grey canvas (`#EDEDF0`). DM Sans typography. Cardless typographic flow (no box containers). Living dot particle system for Self indicator. Two themes: Signature (emerald buttons) and Focus (B&W, no color). Ribbon attempted and abandoned (implementation complexity). All 17 prototypes aligned. `.impeccable.md` updated as authoritative design spec.
  - **Design system changes applied across all prototypes:**
    - Colour palette: `--vivid: #059669`, `--vivid-deep: #3D5A48`, `--vivid-subtle: #ECFDF5`
    - All gradient buttons replaced with flat `var(--vivid)` backgrounds
    - Signature/Focus theme toggle added to proto-nav in all files
    - Sidebar active state: straight `border-left: 2px solid var(--vivid)` (no card, no border-radius)
    - Dot particle system on P00 and P08a input fields (canvas-based, IntersectionObserver for perf)
    - P08 CTA updated from gradient to `var(--vivid)` pill button
    - Text muted darkened to `#65656F` for WCAG AA compliance
    - Accessibility audit completed (14/20): P0 zoom blocking fixed, semantic landmarks noted for future
  - **Current prototype status:** All 17 prototypes fully aligned to design system. P08a is the reference (v7). **Phase 5 design system refactoring complete (2026-03-26):** P09-P20 all refactored to v2 — cardless typographic flow, theme toggle (Signature/Focus), dot particle system, centred 720px layout (where applicable), pill-shaped buttons, Focus theme CSS, reduced-motion support. Compliance table updated in PLAN.md. Reviewer spot-checked P09, P14, P16, P20: PASS. All review flags resolved: sender labels removed from P16/P20 per .impeccable.md spec; P16 message classes normalised to `msg.ditto`/`msg.user` pattern; theme toggle separator normalised to `border-left` across all 7 affected prototypes.
  - **Process visualization analysis (2026-03-26):** Designer evaluated linear narrative (v45) vs node-based DAG (v44) for process detail. Decision: linear narrative stays for intra-process ("How it works"), node graph reserved for inter-process Process Graph (Primitive 14, Month 3+). Anti-persona test confirmed: DAG approach builds for Workflow Designer, not outcome owners. Indented sub-path branching proposed for complex single-process flows (needs prototyping, max 1 level). Spec at `docs/research/process-visualization-approach-ux.md`. Reviewed: PASS WITH FLAGS (3 flags: branching complexity ceiling, Orient-to-Define transition gap, animation detail needed). Insight-103 captured.
  - **Phase B-D prototypes complete (2026-03-26):** All 5 new prototypes designed and built:
    - **P23 v3:** Theme toggle + dot particles added to States 1 (Scanning) and 5 (Success).
    - **P22 v1 (Knowledge in Output):** 4 states — output with provenance summary, provenance expanded (full trail), editing with provenance (edit-as-teaching), clinical view (Jay's session notes with HRV data). Libby + Jay personas.
    - **P21 v1 (Mobile Workspace):** 390px phone sim. 4 tabs — Feed (morning brief + activity), Review (provenance strips + "edit @ desk"), Capture (voice/camera/attach + auto-routing), Process view (narrative steps + trust upgrade prompt). Rob Month 2+, 4 processes.
    - **P14a v1 (Review Queue):** Lisa's dedicated review. 8 items across 3 processes (Content, Pricing, Service). Pre-review checks, pattern detection ("3rd wrong-size issue"), batch actions, urgent filter. 2 states.
    - **P27 v1 (Process Flow Map):** Jordan's inter-process graph (Insight-103). CSS grid nodes, detail panel (340px aside), impact view. 4 process nodes across 4 departments. 3 states.
    - **P29 v1 (Process Model Library):** Rob's capability catalog (Insight-099). Self recommends based on intake context. Browse with categories, model detail (app bindings, quality standards, social proof), setup with detected connections. 4 states.
    - PLAN.md compliance matrix updated (22 prototypes). Index.html updated with all new entries.
  - **Architecture review (2026-03-27):** Dev Architect verified engine can support all prototyped UI. Found 3 gaps in ContentBlock vocabulary — resolved by adding ChecklistBlock, ChartBlock, MetricBlock (16 → 19 types). Right panel gains `"blocks"` PanelContext type for Self-composed structured content. ADR-021 addendum written. Type-check passes.
  - **Prototype system overhaul executed (2026-03-27):** Both plans (5-phase refinement + 8-phase overhaul) merged and executed:
    - **P00 v2 (Workspace Shell):** Complete rewrite. Three-column layout. Sidebar: Today, Inbox (badge), Work, Projects, Routines, Settings — all user language per Insight-073. Visual Vocabulary CSS (badges, field tables, checklists, metrics, sparklines, donut charts, progress bars, suggestion/alert blocks, provenance strips). Two states: empty + populated.
    - **P30 (JSON-Render Composability):** NEW. All 19 ContentBlock types rendered with JSON preview toggle. Three states: conversation, after interaction, block gallery.
    - **Visual composition pass:** P13, P14, P14a, P15, P19 rebuilt with visual vocabulary (metrics, sparklines, donuts, badges, field tables, checklists).
    - **P09 repurposed:** From duplicate intake to "Second Conversation" — Libby returns next day, continuity visible.
    - **New screen prototypes:** P24 (Inbox — Lisa, urgency grouping, triage donuts), P25 (Tasks — Jordan, 12 active, progress bars, filter tabs), P26 (Agent Team — Nadia, capabilities, trust levels), P31 (Process Health — diagnostics, trends), P32 (Settings — AI connection, trust, integrations), P35 (Improvements — proactive suggestions, evidence-based).
    - **Navigation consistency:** All 28 prototypes linked in single canonical chain. Index.html updated with all entries.
    - **Automated checks:** `pnpm run type-check` passes. No test changes needed (prototype-only work).
    - **Review findings:** 10 PASS, 4 FLAG, 1 FAIL. FAIL: "Processes" in sidebar nav is system language (Principle #3). FLAGS: file format (P24/P26/P31 escaped strings — FIXED), layout spec drift (.impeccable.md says two-panel but prototypes use three-column — needs spec update), index button radius (FIXED), accessibility unverified on some files.
  - **Review flags resolved (2026-03-27):** All 5 flags from Dev Reviewer addressed:
    - **FAIL fixed — System language:** Sidebar nav renamed across all 11 workspace prototypes: Home→Today, Tasks→Work, Processes→Routines. P32 trust section: "Trust & Oversight"→"Review preferences", trust labels→"Check everything / Spot check / Let it run". P27 flow map: "Supervised"→"You review", "Spot-checked"→"Spot check".
    - **FLAG fixed — Layout spec:** `.impeccable.md` Layout section updated from Two-Panel to Three-Column workspace with sidebar nav label table and trust label mapping.
    - **FLAG fixed — P30 data list:** Added `format: "list"` variant to block gallery (3 entries, Rob's correction history).
    - **FLAG fixed — File format:** P13, P09, P14, P14a, P15, P19 escaped format fixed (unescaped + mojibake repaired + line number prefixes stripped).
    - **FLAG fixed — Index button:** `border-radius: 9999px` + hover uses `var(--vivid-deep)` token.
  - **Block vocabulary evolution (2026-03-27):** Audit of all 28 prototypes identified general primitive gaps (not review-specific). 4 changes:
    - **ChartBlock enriched:** Added `size?: "inline" | "small" | "medium" | "large"` for responsive scaling. Fixes "charts too small" — sparklines were hardcoded at 40×16px, donuts at 48×48px.
    - **DataBlock enriched:** Added `annotations?: Record<string, FieldAnnotation>` for per-field provenance, flags, and format hints. Non-breaking.
    - **RecordBlock (NEW):** General structured record used across 8 prototypes — review items, inbox items, tasks, roles, knowledge entries, feed items. Title + subtitle + status + fields (with provenance/flags) + checks + provenance + actions + accent border. Renders as typographic flow (no cards).
    - **InteractiveTableBlock (NEW):** Table with per-row actions, row status, column format hints, selection, batch operations. Distinct from DataBlock (display-only). Used for review batch, task lists, knowledge tables.
    - Union: 19 → 21 types. P30 gallery updated to show all 21. Type-check passes. Two new React renderers (`record-block.tsx`, `interactive-table-block.tsx`) registered in block-registry.
  - **Prototype Completion Plan (2026-03-27) — ALL SPRINTS COMPLETE:**
    - **Sprint 1 — Artifact Prototypes (ADR-023):**
      - **P36 Document Artifact (v1) — APPROVED:** Rawlinsons cost estimate. 4 states: generate → refine copper rate → add contingency → approve & send. Three-column workspace (sidebar + conversation + 420px artifact panel). ArtifactBlock shows Created/Updated/Resolved lifecycle. Diff highlighting in right panel. Version bar (v1/v2/v3). Provenance strip. Teach-this moment.
      - **P37 Content Pack (v1) — BUILT:** Steven Leckie Dubai Marina penthouse. 4 states: pack generated (6 pieces) → individual review (Post 2 caption) → batch progress (4/6 approved) → complete (all scheduled). Tabbed right panel preview (Listing/Post 1-3/Script/Email). Phone-frame Instagram previews. Pack grid with approval status.
      - **P38 Code Artifact (v1) — BUILT:** Abodo Wood Epicor ERP integration. 4 states: generation → refinement (add treatment field) → testing (4 pass, 1 warning) → deployment (staging). Dark code block with Catppuccin syntax highlighting (JetBrains Mono). Line-level diff highlighting. Test results panel.
      - **P39 Email Artifact (v1) — BUILT:** Henderson plumber follow-up. 3 states: draft → tone adjustment (casual → formal) → sent. macOS mail client preview styling. Diff highlighting on tone changes. Teach-this for commercial client tone preference.
      - **P40 Clinical Notes (v1) — BUILT:** Jay/Status longevity consultation. 4 states: transcription (voice → structured) → reorganisation (cortisol to assessment) → addition (Vitamin D3) → saved. Animated voice wave indicator. Blood panel grid with badges. Moved content indicator (blue border). Teach-this for section preferences.
    - **Sprint 2 — Existing Fixes:**
      - **F1 P15 Knowledge Base — REBUILT:** Three-column workspace with InteractiveTableBlock. Health strip hero (4 metrics + sparkline). Sortable table (Status/Name/Type/Freshness/Used By/Actions). 12 rows. Click-to-detail in right panel with RecordBlock.
      - **F2 P09 encoding — PREVIOUSLY FIXED** (Phase 5 file format pass).
      - **F3 P11/P12 block vocab — PREVIOUSLY FIXED** (Phase 5 overhaul).
      - **F4 P21 Mobile — REBUILT:** Conversation-forward redesign. 5-tab bottom nav (Today/Inbox/Conversation[primary]/Work/More). Chat as default surface. Block vocabulary CSS. Emerald buttons. Trust labels updated.
    - **Sprint 3 — Polish:**
...
8b. **Planned:** Process Model Library (Insight-099) — abstract action taxonomy + binding resolution needs ADR when Phase 11 design begins. Extends ADR-005, ADR-008, ADR-019. Transforms the process-analyst's role from blank-canvas articulation to model discovery + binding + customisation.
