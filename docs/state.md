...
- **Reference doc drift (Brief 074):** Trust-gate handler does not yet read `goalTrustOverrides` from processRun inputs — overrides are stored and logged but not enforced at the trust gate. **Remaining action:** Architect to design trust-gate handler integration for goal-level overrides.
...
- **Reference doc drift (Briefs 072+073+074):** Six docs need Architect updates: (1) `architecture.md` — L1 block inventory (+3 interactive types), L2 orchestrator/router implementation, L6 composition engine module. (2) `human-layer.md` — interactive blocks, intentContext, composition functions. (3) `ADR-021` — FormSubmitAction type, form-submit routing tokens. (4) `ADR-010` — router implementation (slug+keyword matching), pause_goal, goal auto-wiring lifecycle. (5) `ADR-024` — block inventory 21→24, intentContext, composition-engine.ts references. (6) `dictionary.md` — 9 new terms (ConnectionSetupBlock, FormSubmitAction, goalHeartbeatLoop, intentContext, InteractiveField, matchTaskToProcess, pauseGoal, resumeGoal, WorkItemFormBlock). **Remaining action:** Architect session to batch-update all 6 docs.
