...
**Last updated:** 2026-05-11 (Brief 266 — Client-lane suggested-candidates panel, parent/per-card actions, staleness refresh race-disable, and workspace upsell debug choreography implemented + reviewed; pending human approval. Real intro emission remains Brief 261; real off-network scout remains Brief 258.)
**Previously:** 2026-05-09 (Brief 263 — Network Tier Postgres Migration to Supabase complete; foundation steps 262 + 263 done; sub-briefs 255 + 256 implemented & pending approval; 257-261 pending. Network tier now on Postgres via `networkDb` + `@ditto/core/db/network`; workspace tier unchanged on SQLite. ADR-048 accepted; ADR-036 §1+§3 accepted, §2 superseded-in-part. Dual migration tree: `drizzle/` (workspace SQLite) + `drizzle/network/` (Postgres).)
