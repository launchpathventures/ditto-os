...
**Last updated:** 2026-05-12 (Brief 258 — Knowledge-base intake and source-grounded off-network scout implemented + reviewed PASS; pending human approval. Expert lane now has markdown-backed KB upload/voice/manual/private-filter shelf with per-fact visibility and audit. Client lane scout now calls a guarded `scout_off_network` route and renders source-backed scouted candidates without budget/private-filter leakage. Real public profile/share/intro emission remains Briefs 259/260/261.)
**Previously:** 2026-05-11 (Brief 266 — Client-lane suggested-candidates panel, parent/per-card actions, staleness refresh race-disable, and workspace upsell debug choreography implemented + reviewed; later human-approved by PM on 2026-05-12. Real intro emission remains Brief 261.)
...
| Sub-brief 258 — KB intake (voice + upload + per-fact visibility) + 4 new schema tables + off-network scout tool | pending; depends on 257 | Brief 254 unlock | Ethos v33 (voice pattern); per-fact visibility = original to Ditto | TBD |
