...
| Sub-brief 264 — Client-lane schema + six-question intake + on-network match endpoint + `IntroRequestStub` typed contract | implemented; pending human approval | Brief 257 split | Original; network Postgres per ADR-048 | `JobRequestCardBlock`, `networkJobRequests`, `/api/v1/network/match`, `matchOnNetwork` |
| Sub-brief 265 — `JobRequestCardRenderer` + preview-pane client branch + mobile tap-to-edit overlay + `FitConfidenceDot` | implemented + reviewed; pending human approval (2026-05-11) | Brief 257 split | Designer spec `network-client-lane-ux.md`; existing profile-card visual language | `packages/web/app/network/chat/job-request-card-renderer.tsx`, `preview-pane.tsx`, `fit-confidence-dot.tsx` |
| Sub-brief 266 — Suggested-candidates panel + action row + staleness refresh + upsell debug choreography | implemented + reviewed; pending human approval (2026-05-11) | Brief 257 split | Designer spec `network-client-lane-ux.md`; Ditto stub choreography | `packages/web/app/network/chat/suggested-candidates-panel.tsx`, `client-card-actions.tsx`, `workspace-upsell.ts` |
...
| Sub-brief 259 — `/people/[handle]` profile-as-chat + Ask-Greeter system prompt with **6 hard-rule unit tests** + visitor rate limit + voice mode | pending; depends on 256 + 258 | Brief 254 unlock | process-os Ask Charlie (adapt; ID rule INVERTS) | TBD |
