...
**Last updated:** 2026-05-19 (Documenter — **Brief 286 `/dev-reviewer` fix-all recorded; fresh-context re-review APPROVE; pending human approval.** Six findings closed across workspace-mode admin API hard-404, global-pause send guards, fail-closed pause UI, action totals, structured reason validation, and anti-persona metadata scrub.)
**Previously:** 2026-05-19 (Documenter — **Brief 279 closeout recorded; pending human approval.** Brief status/ACs/completion notes/state/roadmap now reflect final fresh-context APPROVE, source-row privacy/export/delete/retention fixes, and the residual non-blocking outbox gap after provider acceptance.)
**Previously:** 2026-05-19 (Documenter — **Brief 285 polish review recorded; pending human approval.** `/dev-reviewer` returned PASS with flags for the Privacy Center design-recommendation polish: untracked-file handoff risk and missing rail-state test assertions; no privacy/route/side-effect regression.)
**Previously:** 2026-05-19 (Builder — **Sub-brief 286 implemented & fresh-context reviewed APPROVE; pending human approval.** Added shared fixed-window Network abuse controls with `network_rate_counters`, the admin health dashboard/dry-run/audited reveal/override surface, route integrations for current search/profile-chat/privacy challenge/share/intro seams, and bounded-visibility tests. First review returned REVISE for visitor/profile limiter + request-pause gaps; both fixed and re-reviewed APPROVE. Watch-specific producer/enforcement remains dependency-bound to Brief 275.)
**Previously:** 2026-05-18 (Documenter — **Sub-brief 283 follow-up closeout recorded.** Brief 283 status, AC checklist, completion notes, roadmap row, and state snapshot now reflect the review-fix patch: transactional complaint-webhook dedup + enforceable suppression uniqueness. No new ADR/insight needed; the lesson is folded into the Brief 283 retrospective.)
**Previously:** 2026-05-18 (Builder — **Sub-brief 283 review-fix patch applied.** Complaint webhook dedup claim + complaint handling now run in one network DB transaction, so handler failure rolls back the dedup claim for Svix retry recovery; suppression uniqueness now uses partial unique indexes + a scope/user check constraint so global rows with `scope_user_id IS NULL` are enforceably unique. Verification: focused Brief-283 vitest 93/93, root type-check pass, `git diff --check` pass; full-suite timeout files reran 27/27 pass.)
**Previously:** 2026-05-18 (Builder — **Sub-brief 282 (Network audit / scrubber / stop-run substrate) IMPLEMENTED & fresh-context reviewed APPROVE after `/dev-review` fixes; pending human approval.** Added append-only `network_audit_events` with migration idx 9, server-minted wrapper-run audit validation, central eight-surface privacy scrubber, block/share/OG anti-persona safeguards, and strict server-minted `generateShareVariants` guard. Verification: focused vitest 47/47, `pnpm run type-check` pass, full `pnpm test` 3065/3065 (13 skipped), `git diff --check` pass, and `pnpm test:e2e:auto` pass; broad Playwright e2e still has unrelated baseline failures.)
**Previously:** 2026-05-18 (Documenter — **Brief 273 (Need Request onboarding + manual-search entry) COMPLETE & APPROVED.** Review blockers fixed under "fix all": request UI now persists calibrated identity and `requestId`; repeated saves update existing Active Requests; `update_need_request` derives actor ownership from execution context and exposes no owner-id fields in the model schema; raw-need drafting has untrusted-input prompt boundaries; seeded request handoff no longer waits 45s; `/network` research handoff e2e is deterministic; stale Brief-274 core import fixed; full-suite baseline drift closed via provisioning mock completion + network-tier test isolation. Verification: focused Brief-273 vitest 96/96, landing Playwright 8/8, root + web type-check 0 err, full `pnpm test` 3037/3037, `git diff --check` clean. Brief moved to `docs/briefs/complete/`.)
**Previously:** 2026-05-18 (Documenter — **Brief 281 (Workspace Artifact Recall & Archive) COMPLETE & APPROVED.** Builder + fresh-context maker-checker (APPROVE-WITH-NITS, 16/16 ACs PASS) followed by a `/dev-review` 5-pass audit (7 findings) → all 7 fixed under "fix all": memory N+1 batched via `inArray`; single `ProjectFilter` resolved once & threaded across all kinds; real focus trap + restore in the Archive drawer; memory scope/type threaded for citation scope-pill fidelity; `inArray`/`and` imports; `" none"` slug sentinel removed; ARIA corrected to `list → listitem → link`. Verification: root + web type-check clean for Brief-281 files, focused vitest 194/194, Playwright `workspace-artifact-recall.spec.ts` 3/3. Insight-236 captured (a shared multi-kind recall helper resolves the project filter once and applies it uniformly, omitting non-project-scopable kinds). Remaining web tsc `@ditto/core/content-blocks` error belongs to the parallel Brief-274 session — out of 281 scope.)
**Previously:** 2026-05-16 (Documenter — **Brief 280 (Workspace Conversational Front Door / Self-First IA inversion) COMPLETE & APPROVED.** Builder + fresh-context maker-checker review (verdict APPROVE WITH NITS); AC8 trust-tier-label fix applied + regression-pinned; both non-blocking observations resolved (Insight-235 captured for the transport-vs-runtime-filter seam; orphaned `workspace-page.tsx` deleted). Verification: root type-check 0 errors, focused vitest 74/74, Playwright onboarding + workspace-chat-front-door 6/6. Brief 281 (artifact recall/archive) is the drafted dependent follow-on, now unblocked. Also: `docs/state.md` compaction pass executed this session per the standing doc-hygiene blocker.)
**Previously:** 2026-05-16 (Architect/Documenter — Briefs 280-281 scoped as a two-step Workspace UX correction. Brief 280 is design-ready for `/chat` as authenticated Self home; Brief 281 is drafted for artifact recall/archive search after 280 lands.)
**Previously:** 2026-05-14 (Reviewer/Architect fix pass — Network Superconnector Reframe brief chain corrected before build. Parent Brief 270 plus child Briefs 271-279 now make the economic-outcome thesis explicit, require `278 foundation` before production discovery/claim invites, constrain LinkedIn to pointer/user-provided/consented/API-only use, separate manual search/background watch/outbound discovery, require wrapper-step-run protections on all side-effecting seams, and remain draft pending fresh-context review + human approval.)
**Previously:** 2026-05-13 (Brief 261 built; review fixes applied; pending fresh-context re-review + human approval. Introductions now have Network-tier storage, guarded `emit_intro_request`, client/visitor HTTP bypass rejection including falsy values, free-counter `costLabel`, durable workspace inbox delivery, refusal triggers, bounded blocklist patterns, Q6 upsell idempotency, and self-service workspace-provision route. Parent Brief 254 remains open until review/approval closeout.)
**Earlier:** 2026-05-13 (Brief 259 complete after Builder + Reviewer fix loop + Documenter closeout. Public `/people/[handle]` profile-as-chat, prompt-driven representative replies, durable Network → workspace inbox delivery queue, workspace import/action routes with terminal-state persistence, server-side visitor intro session gating, scoped `forward_note_to_user`, and voice capture fallback are in place. Insight-234 captured and the architecture review checklist now gates cross-deployment delivery durability. Next parent-254 work: Briefs 260 and 261.)
**Earlier:** 2026-05-13 (Brief 267 healthz middleware hotfix shipped (PR #52, commit `7dc00e4` on main) and launchpath managed workspace refreshed end-to-end through the hardened provisioner — service `29085f41-ba0a-4c5d-82f9-5457b10cde89`, status healthy. Insight-233 captured: when two cross-component contracts tighten in the same change, walk public-prefix endpoints endpoint-by-endpoint against the fully-configured env, not just the failure-closed env. Brief 267 remains complete; the hotfix amends `/api/healthz` into `BASE_PUBLIC_PREFIXES` with regression coverage.)
**Earlier:** 2026-05-13 (Brief 259 — reviewer-fix pass implemented; pending fresh-context re-review + human approval. Durable Network → workspace inbox delivery queue, workspace import/action routes with terminal-state persistence, server-side visitor intro session gating, scoped `forward_note_to_user`, prompt-driven visitor replies, and real voice capture fallback are now in place.)
**Earlier:** 2026-05-12 (Brief 258 — Knowledge Base intake and off-network scout complete after Builder commit `4718c64`, Reviewer PASS WITH FIXES → final PASS, human approval, and Documenter closeout. Four new network-tier Postgres tables, three guarded built-in tools, KB upload/visibility/voice/scout API routes, expert KB shelf UI, source-grounded scout candidates with public URL fail-closed, and audited HTTP-route wrapper step run pattern captured as Insight-232.)
**Earlier:** 2026-05-11 (Brief 266 — Client-lane suggested-candidates panel, parent/per-card actions, staleness refresh race-disable, and workspace upsell debug choreography implemented + reviewed; later human-approved by PM on 2026-05-12. Real intro emission remains Brief 261.)
**Current phase:** Phase 9 **in progress** (Recursive Self-Improvement — Briefs 189-196 ready; 189 builder-claimable). **User-Facing Legibility phase in progress** (2026-04-20) — parent Brief 197 + sub-briefs 198/199/200 ready; 198 + 200 builder-claimable in parallel, 199 waits for 198. Phase 10 **complete**. Phase 14 **in progress** (Network Agent — engine code complete, Briefs 071+097+098a+098b+099a+099b+099c+108+110+114+119+120+121+124+125+126+127+128+129+130+131 all complete. Brief 115 Operating Cycle Archetype designed — split into sub-briefs 116-118, all complete. Briefs 119-141 complete. GTM pipeline fully tooled). Phase 15 **complete** (Managed Workspace Infrastructure — Briefs 090+091+100+153+267 complete). Phase 11 **in progress** (Briefs 101-104+107+135 complete; browser-write Briefs 182-185 ready, 183 builder-claimable). Phases 12-13 **future**. 1797 unit tests + 14 e2e tests (4 spec files). 32 process templates. Front door conversational experience live. **Meta-process robustness: ALL COMPLETE (2026-04-16).** All 10 meta processes (MP-1 through MP-10), 44 work items, 11 briefs (145-148, 155-165). See `docs/meta-process-roadmap.md`. **User-journey P0 residuals closed (2026-04-16):** Brief 169 parent + sub-briefs 170-178 landed, covering shell-injection, credential-leak via tool output, budget pre-dispatch guard, YAML round-trip validation, definitionOverride cleanup, memory dropout observability, briefing query efficiency, ambiguous-intent clarification, stale-escalation ladder primitive. Dev-reviewer: CONDITIONAL PASS, follow-ups tracked in Brief 179 (3 P0 + 4 P1).
...
| ~~Brief 219 — Greptile/Argos integrations~~ | **RETIRED 2026-04-27** | — | User steer: drop entirely. Default review = `/dev-review` skill in Routine + human PR review (sub-brief 216). Default visual = Vercel preview URL inline card from `deployment_status` non-production events (sub-brief 216). No third-party AI-review/visual-diff SaaS dependency. |
| Brief 220 — Deploy Gate + GitHub Mobile | done (2026-04-28, post-Builder + Reviewer-pass + Reviewer-fix second pass) | `docs/briefs/complete/220-deploy-gate.md` | Original — `briefStateValues` extended with `deploying / deployed / deploy_failed`; engine-core pure module `transitionBriefState()` at `packages/core/src/work-items/state-machine.ts` (mirrors runner-dispatch state-machine shape) admits out-of-order webhook delivery (`shipped → deployed | deploy_failed` direct) + retry-out-of-order (`deploy_failed → deployed`) + `shipped → archived` for non-deploy-gated projects. `cloud-runner-fallback.ts:processProductionDeployStatus()` replaces the prior `production-no-op` outcome with five differentiated `FallbackOutcome` variants (`deploy-approval-pending` / `deploy-in-progress` / `deployed` / `deploy-failed` / `deploy-state-no-op`); per-state branch with idempotent-replay audit (`metadata.transitionRejected = true`) + bounded-waiver pattern (`metadata.guardWaived = true` per Insight-180 — webhook has no `stepRunId`). `buildMobileApproveAction()` URL composer with `^[a-zA-Z0-9._-]+/[a-zA-Z0-9._-]+$` regex sanitisation + graceful `/deployments` fallback when `workflow_run_id` is absent. Status route extended with `briefStateToDispatchEvent` cases returning `null` for the 3 new states + state-machine validation at the route boundary returning **409 Conflict** on illegal transitions (Reviewer-fix M4 second pass). `DeploymentStatusEvent.deployment` typed shape widened (`id?` + `workflow_run_id?`). Lookup uses `findDispatchForRepo({ includeTerminal: true })` directly so the most-recent-by-createdAt row wins regardless of status (Reviewer-fix H2 + M2 — production deploys fire post-runner-completion when the dispatch is `succeeded`). Template `docs/runner-templates/deploy-prod.yml` (Vercel default + Netlify/Cloudflare/Fly commented alternatives) + companion runbook `deploy-prod-setup.md` (GitHub Environment + Required Reviewer + GitHub Mobile push setup + first-deploy smoke + troubleshooting); bash-syntax CI gate (`deploy-prod-setup.test.ts` runs `bash -n` on every fenced bash block — caught `<OWNER>/<REPO>` as bash-invalid; runbook now uses `$OWNER/$REPO` shell vars). Architecture amendment at `docs/architecture.md` §L3 documents the deploy-gate primitive. Dictionary entries: Deploy Gate, Deploying State, Deployed State, Deploy Failed State, Deploy-Prod Workflow Template, GitHub Environment, Required Reviewer (7 entries). 108 Brief-220-specific tests across 4 files (state-machine 30 + cloud-runner-fallback 53 + route 20 + runbook bash-syntax 5). **No DB migration shipped** — `briefState` is plain `text` with no enum CHECK constraint at the DB level (Brief 223 enforces via Zod app-validation only); idx-17 reservation released. **AC #14 (E2E smoke against agent-crm) deferred manual; AC #15 (mobile-first conversation-surface card rendering) deferred to sub-brief 221.** |
| Brief 221 — Runner Mobile UX — approval flow + inline-card substrate (structured `/review/[token]` with vertical-radio "Run on:" selector + force-cloud toggle + sticky-bottom Approve/Reject; `pauseRunnerDispatchForApproval()` first production caller of `dispatchWorkItem`; engine-core `mintRunnerDispatchPause()` + `buildRunnerDispatchCard()` helpers; `StatusCardBlock` gains optional `metadata?` + discriminator-keyed renderer via `metadata.cardKind`; `WorkItemFormBlock` gains optional `formId?`; `activities.contentBlock` JSON column for inline-card persistence; namespace-bypass-safe approve route with `selectedKind` honoured via `runnerOverride` + token consumed only on success; mobile-viewport e2e at 320×568 + 375×667) | done (2026-04-28, post-Builder + Reviewer + dev-review fixes; 5 commits on branch awaiting merge) | `docs/briefs/complete/221-runner-mobile-ux.md` (+ Designer spec at `docs/research/runner-mobile-ux-ux.md`) | Original — selector-at-approval-time + manual-retry-of-chain-advance are design gaps (no CI/deploy tool surfaces this at approval); patterns adopted from GitHub Actions / Vercel / iOS HIG |
| Brief 231 — Runner Admin UX (pill on `/projects/[slug]` recent-dispatches list + per-project + workspace `/admin` runner-metrics + retry-next-in-chain API + button on inline cards + full `<DispatchCard>` template + bridge-dispatch migration to shared helper) | not yet written (deferred from Brief 221 buildability narrowing 2026-04-28; Architect-blocked) | — | Original — same-track sibling sub-brief; consumes Brief 221 substrate (StatusCardBlock metadata + cardKind discriminator + activities.contentBlock + RunnerPillView leaf already shipped) |
...
| Brief 228 — Project Retrofitter — `.ditto/` Substrate Writer (sub-brief #3a of 224) | done (2026-04-28, post-Builder + Reviewer (REVISE → all 2 CRIT + 5 IMP + 8 MIN findings fixed)) | `docs/briefs/complete/228-project-retrofitter.md` | Ships the retrofit pipeline for autonomous + critical + spot_checked tiers (supervised tier renders a placeholder with escalate-to-autonomous CTA; per-file approval surface deferred to Brief 229). 4-step process at `processes/project-retrofit.yaml` (`generate-plan` → `surface-plan` → `dispatch-write` → `verify-commit`); 4 systemAgent handlers in `src/engine/onboarding/retrofitter.ts` (~1200 LOC, all carry Insight-180 stepRunId guard); runner-agnostic prompt template at `src/engine/onboarding/retrofit-prompt.ts` (payload-by-reference per token-budget discipline); shared `readPriorStepOutputs` extracted to `packages/core/src/harness/step-output-reader.ts` (Insight-217 absorption); 4 new core types (`RetrofitPlan`, `RetrofitFile`, `RetrofitDispatchPayload`, `RetrofitFileAction`); `RetrofitPlanBlock` (28th in the discriminated union) + Tailwind-utility-mapped React renderer covering 5 surfaceable status states + supervised placeholder; new `rerun_project_retrofit` Self tool (36th tool); new `/api/v1/projects/[id]/retrofit` POST route + Re-run button; confirm route TODO breadcrumb at `confirm/route.ts:271-275` replaced with `startProcessRun(..., { parentTrustTier: data.trustTier })`. **ADR-043 written + accepted** (`.ditto/` directory shape: 7 role-contracts/ + skills.json + tools.json + guidance.md + onboarding-report.md + version.txt + .gitignore = 13 files; `# DO NOT EDIT — regenerated by Ditto retrofit (run <id>)` header convention; Catalyst-coexistence stance — `.ditto/` writes never touch sibling directories; Q3 user-edit safety mechanism documented). Trust-tier flow now consumes the previously-dropped `data.trustTier` via `parentTrustTier` option through `startProcessRun` (heartbeat.ts:1742-1753 inheritance discipline). Reviewer CRIT-1 fix: YAML's `trust.initial_tier` set to `autonomous` (NOT supervised — the harness's `moreRestrictiveTrust` would silently downgrade user-picked tiers). Reviewer CRIT-2 fix: 3 queries rewired from `processes.projectId` (always null for system processes) to `json_extract(processRuns.inputs, '$.projectId')`. **AC #11 user-edit-safety SURFACE partial** — runner-side enforcement happens (the prompt instructs the runner to compare on-disk hashes and skip user-edited files), but the runner_dispatches schema has no body channel for the runner's structured `{ commitSha, actuallyChangedFiles, skippedFiles }` response; Ditto parses commitSha from `externalRunId` when hex-shaped, leaves `skippedUserTouchedFiles` empty until a follow-on schema-extension brief lands the `responseBody` column. 55 Brief-228 tests + 13 Brief-226 non-regression all green; full suite 2487/2505 (failures are pre-existing flakes, not Brief 228). 3 dictionary entries added (`Battle-Readiness`, `Project Substrate`, `Retrofit`). |
| Brief 230 — Design-System Component CSS Layer + Block-Renderer Promotion (cross-cutting) | done (2026-04-29, post-Builder verification + separate-context Dev Reviewer (APPROVE) + /dev-review (0 issues across 5 passes) + Documenter close-out; AC #8 visual smoke deferred to manual human walkthrough) | `docs/briefs/complete/230-design-system-component-css-layer.md` | Authors the bundled component-class CSS layer at `packages/web/app/design-system.css` (imported into `globals.css`) — six block primitives from the Anthropic Claude Design package (`block.breakdown`, `block.decision`, `block.plan`, `block.compare`, `block.evidence`, `block.people`) + chrome (`block-head`, `bbody`, `block-foot`, `bkind`, `bopen`) + composition helper (`alex-line`/`alex-mark`/`alex-body`) authored verbatim. **Plus one Ditto-original primitive `.block.findings`** with tone modifiers (`tone-positive`/`tone-caution`/`tone-negative`/`tone-info`/`tone-vivid`) — a sectioned-list-with-tone-coded-header for Strengths/Watch-outs/Missing surfaces, distinct from `.block.evidence`'s kv-pair semantic. Three missing tokens added to `globals.css` `@theme`: `--text-3xl` + `--text-4xl` (with `--*-line-height` companions) + `--color-vivid-subtle-border`. `AnalyserReportBlock` renderer promoted from utility-mapped to bundled-class composition (the gap-flag comment block at `analyser-report-block.tsx:1-24` discharged); JSX structure + Public API preserved. Architecture amendment at `docs/architecture.md` §L6 ("Design-token-vs-component-class layering"). Three new dictionary entries (`Block Primitive`, `Bundled Component Class`, `Design-Token-vs-Component-Class Boundary`). Brief 228 §Open Question 6 resolved. Token-only colour discipline (zero hex literals in component classes); class names match the design package verbatim (no Tailwind-rename). 6 Brief-230 tests pass; type-check clean for Brief-230 files (concurrent-session type errors in unrelated files predate this brief). 25 other renderers intentionally NOT promoted (per-renderer promotion happens organically); workspace shell adoption intentionally deferred (separate follow-on candidate). |
...
| Workspace Conversational Front Door — `/chat` as authenticated post-Day-Zero Self home, existing ContentBlocks inline, legacy primitive pages as drill-downs — **complete (Brief 280, 2026-05-16) — APPROVED; maker-checker PASS, AC8 trust-label fix + 2 non-blocking observations resolved** | Brief 280, ADR-016, ADR-021, ADR-024, Insight-180/183/211/232/235 | Existing `/chat`, `/api/chat` Self stream, ContentBlock protocol, action-boundary tests |
| Workspace Artifact Recall and Archive — chat header archive drawer plus `search_workspace` Self tool for projects/processes/memories/work/reviews — **complete (Brief 281, 2026-05-18) — APPROVED; maker-checker APPROVE-WITH-NITS, then `/dev-review` 7 findings all fixed (memory N+1, single ProjectFilter, focus trap/restore, scope-pill fidelity, ARIA list semantics)** | Brief 281, ADR-016, ADR-021, ADR-024, Insight-211 | Shared read-only recall helper, existing project/memory/process routes, InteractiveTableBlock/RecordBlock/KnowledgeCitationBlock |
...
**Status:** Engine code complete (Briefs 080-085). Process templates complete. Character bible + persona architecture complete. Front door + intake flow complete. **Web acquisition funnel complete (Briefs 093-095):** conversational front door, verify page, referred page, post-submission engagement. **Referred surface refocus complete (Brief 243, ADR-041):** locked Alex/Mira continuation from `personaId`, `ref`/`introducerId` URL context, learned context from message zero, workspace redirect for existing users. **Marketing IA refocus complete (Brief 244, ADR-041):** `/chief-of-staff` retired in favor of `/your-ditto`, 301 redirect added, marketing nav/footer updated, and public marketing copy aligned to Brand/Greeters/Self ontology. **First-time visitor canned-opener parity complete (Brief 241, Insight-223, Insight-224):** UX spec §5.1 openers render instantly on persona pick (no LLM round-trip), single source of truth `greeterOpenerMessages()`, server-side session seed with `[INTERVIEW_BEGIN]` user marker for Anthropic alternation, and speculative prompt-cache priming via `primeInterviewCache()` cuts ~1.5-2s off first-reply latency. **Multi-provider purpose routing complete (Brief 096, ADR-026):** Anthropic + OpenAI + Google simultaneous loading, purpose-based model selection. Deployment briefs (086-089) designed and approved. **Integration tools complete (Brief 097):** CRM built-in tools, atomic send-and-record. **Pulse + chain execution complete (Brief 098a).** **Inbound email + status composition complete (Brief 098b).** **Communication intelligence complete (Brief 099a/b/c).** **Referral footer complete (Brief 109).** **Admin oversight complete (Brief 108).** **Workspace suggestion trigger complete (Brief 110).** **Operating Cycle Archetype complete (Briefs 115-118).** **Pricing strategy complete (Brief 119).** **Two-sided front door (Brief 254 chain) mostly built:** 262/263/258/259/264/265/266 are done, 260 is implemented and reviewed PASS, and 261 is built with review fixes pending fresh-context re-review/human approval. **Network Superconnector Reframe (Brief 270 + 271-279 plus 278 substrate) is in buildout:** Briefs 272-274 and 282-286 are implemented/reviewed, Brief 279 is implemented and fresh-context reviewed APPROVE pending human approval, Brief 287 remains the closeout checkpoint, and Briefs 275-277 remain draft for the next Network sequence.
...
| 15 additional templates (shared, Greeter connecting/nurturing, agency-mode Gear 1+2 acquisition) | done | Insight-153, ADR-041, Brief 240 | Original | `processes/templates/` |
...
| First-time visitor canned greeter opener (UX spec §5.1, Brief 241) | done | Brief 241, Insight-223, Insight-224 | Original to Ditto | `packages/web/lib/persona.ts:greeterOpenerMessages`, `packages/web/app/welcome/ditto-conversation.tsx`, `packages/web/app/api/v1/network/chat/persona/route.ts`, `packages/web/app/api/v1/network/chat/prime-cache/route.ts`, `src/engine/network-chat.ts:primeInterviewCache` |
| First-time visitor staged preamble (cursor + 3 fade-in lines → Tripoli v2 hero) | done | Brief 253 (conversational) | Verbatim port of commit 1b6b63e cadence | `packages/web/app/welcome/ditto-conversation.tsx` (firstTime/preamble state, timer effect, Skip + Escape, JSX) |
| Returning desktop visitor wedge autoplay | done | Brief 253 follow-up (conversational) | Original | `packages/web/components/marketing/wedge.tsx` (`autoStart` prop, one-shot mount effect, `wedge_autoplay` analytics), `packages/web/app/welcome/ditto-conversation.tsx` (`autoStartWedge` state, viewport+visited gating), `packages/web/lib/marketing-analytics.ts` (`wedge_autoplay` event). Reduced motion + hidden tab opt out. |
| Anti-enumeration verify page | done | Brief 095 | Magic Link pattern | `packages/web/app/verify/page.tsx` |
| Recipient-to-user referred page | done | Brief 095; Brief 243 (complete) | Referral landing pattern + ADR-041 locked-Greeter continuation | `packages/web/app/welcome/referred/page.tsx`, `packages/web/app/welcome/referred/referred-experience.tsx` |
| Marketing IA refocus (`/chief-of-staff` -> `/your-ditto`) | done | Brief 244, ADR-041 | Next.js redirect + existing marketing layout (adopt) | `packages/web/app/your-ditto/page.tsx`, `packages/web/next.config.ts`, `packages/web/components/marketing/site-nav.tsx`, `packages/web/components/marketing/site-footer.tsx` |
| **Two-Sided Conversational Front Door (Brief 254 parent + Briefs 255-263 sub-briefs)** | | | | |
| `/network` parent brief (revision 2.2) — Ethos-pattern landing, two lanes, profile-as-chat, KB intake, share modal, intro authorization. Build order: 262 → 263 → 255 → 256 ∥ 258 → 257 → 259 → 260 → 261 | spec-complete (reviewer-pass fixes applied) | Brief 254 | Ethos (pattern), process-os Ask Charlie (adapt with REPRESENTATIVE-not-IMPERSONATOR inversion), Brief 248 (extend) | `docs/briefs/254-network-two-sided-conversational-front-door.md` |
| Sub-brief 262 — Network/Workspace Tier Reclassification (foundation step 1): move `reviewPages` → `harness.ts`, `documents`+`documentContent` → `knowledge.ts`; `network.ts` reduced to 8 sqliteTable | done (2026-05-09) | Brief 254 unlock; ADR-036 §1 alignment | Original | `src/db/schema/{harness,knowledge}.ts`, `src/db/schema/network.ts` (8 tables), `src/db/schema/index.ts` barrel |
| Sub-brief 263 — Network Tier Postgres Migration (foundation step 2): schema split to `packages/core/src/db/network/` (`pgTable`), `networkDb` via postgres-js, dual migration tree (`drizzle/network/`), boot-time `ensureNetworkSchema()`, slow-query log, graceful 503, `withNetworkDbTransaction` test helper | done (2026-05-09) | ADR-036 §3 + ADR-048 (supersedes ADR-036 §2) | postgres-js (depend), drizzle-orm/postgres-js (depend), Supabase Postgres (host) | `packages/core/src/db/network/{schema,index}.ts`, `src/db/network-db.ts`, `src/db/network-db-test-helpers.ts`, `drizzle.network.config.ts`, `drizzle/network/`, `packages/web/lib/network-availability.ts` |
| Sub-brief 255 — Ethos single-viewport landing + bottom toggle + persona-neutral copy + ChatContext extension + Instrument Serif font loading | implemented; pending human approval | Brief 254 unlock | Ethos v30/v31 (pattern), `next/font/google` (depend) | `/network`, `/network/chat`, `NetworkLanding`, `NetworkCardPreview`, `initializeNetworkLaneSession`, `ChatContext expert/client` |
| Sub-brief 256 — Expert lane + new `NetworkProfileCardBlock` + `handle` field + 6-question directive + live-preview pane + handle-claim | implemented; pending human approval | Brief 254 unlock | Ethos v32 (pattern); engine boundary: `packages/core/src/content-blocks.ts` | `NetworkProfileCardBlock`, `networkUsers.handle/card`, expert intake helper, `/api/v1/network/handle`, in-chat card renderer |
| Brief 257 — Client lane parent design reference split into sub-briefs 264/265/266 | split parent; implementation tracked below | Brief 254 unlock | Original; budget-privacy is Soul Move | `docs/briefs/257-client-lane-job-request-and-on-network-match.md` |
| Sub-brief 264 — Client-lane schema + six-question intake + on-network match endpoint + `IntroRequestStub` typed contract | done (2026-05-12) | Brief 257 split | Original; network Postgres per ADR-048 | `JobRequestCardBlock`, `networkJobRequests`, `/api/v1/network/match`, `matchOnNetwork` |
| Sub-brief 265 — `JobRequestCardRenderer` + preview-pane client branch + mobile tap-to-edit overlay + `FitConfidenceDot` | done (2026-05-12) | Brief 257 split | Designer spec `network-client-lane-ux.md`; existing profile-card visual language | `packages/web/app/network/chat/job-request-card-renderer.tsx`, `preview-pane.tsx`, `fit-confidence-dot.tsx` |
| Sub-brief 266 — Suggested-candidates panel + action row + staleness refresh + upsell debug choreography | done (2026-05-12) | Brief 257 split | Designer spec `network-client-lane-ux.md`; Ditto stub choreography | `packages/web/app/network/chat/suggested-candidates-panel.tsx`, `client-card-actions.tsx`, `workspace-upsell.ts` |
| Sub-brief 258 — KB intake (voice + upload + per-fact visibility) + 4 new schema tables + off-network scout tool | done (2026-05-12) | Brief 254 unlock | Ethos v33 (voice pattern); per-fact visibility = original to Ditto; source-grounded scout adapts `webSearch` from Brief 093 | `packages/core/src/db/network/schema.ts` (13 pgTable), `drizzle/network/0002_broad_mac_gargan.sql`, `src/engine/network-kb-{extract,storage,context,feedback}.ts`, `src/engine/network-voice-intake.ts`, `src/engine/network-scout.ts`, `src/engine/network-step-run.ts`, `src/engine/tool-resolver.ts`, `packages/web/app/api/v1/network/kb/{upload,visibility,voice}/route.ts`, `packages/web/app/api/v1/network/scout/route.ts`, `packages/web/app/network/chat/network-kb-shelf.tsx`; Insight-232 |
| Sub-brief 259 — `/people/[handle]` profile-as-chat + Ask-Greeter system prompt with **6 hard-rule unit tests** + visitor rate limit + voice mode | done (2026-05-13) | Brief 254 unlock; `docs/briefs/complete/259-public-profile-as-chat-and-representative-rule.md` | process-os Ask Charlie (adapt; ID rule INVERTS), Brief 248 auth block (adopt), Insight-231 cross-deployment contract, Insight-234 durable outbox | `/people/[handle]`, `/api/v1/network/people/[handle]/{chat,intro-request}`, `/api/v1/network/workspace-deliveries`, `/api/v1/workspace/inbox/{import,authorization}`, `src/engine/network-chat-prompt.ts`, `src/engine/forward-note-to-user.ts`, `src/engine/visitor-rate-limit.ts`, `src/engine/visitor-profile-chat.ts`, `src/engine/workspace-inbox-delivery.ts`, `packages/core/src/db/network/schema.ts`, `drizzle/network/0003_create_network_forwarded_notes.sql`, `drizzle/network/0004_create_network_workspace_deliveries.sql` |
| Sub-brief 260 — Share modal + 3 voices (quiet/loud/ask) + dynamic OG renderer + downloadable PNG | design-ready + reviewed (2026-05-13); pending human approval; depends on 256 + 258 + 259 + 248 | Brief 254 unlock | Original; iconic-card-in-4-contexts is Soul Move #7; `next/og ImageResponse` (depend — already in tree via Next.js 15.3.3) | `docs/briefs/260-network-share-modal-og-and-png.md` |
| Sub-brief 261 — Introductions primitive + free counter + `AuthorizationRequestBlock.costLabel` population + workspace upsell verbatim | built (2026-05-13); review fixes applied; pending fresh-context re-review + human approval | Brief 254 unlock | Brief 248 (`costLabel` already on primitive); Brief 259 cross-deployment seam (adopt); AC-J 4-trigger taxonomy + upsell idempotency log (original) | `introductions`, `network_user_block_list`, `network_session_upsell_log`, `emit_intro_request`, `/api/v1/network/intros`, `/api/v1/network/people/[id]/intro-request`, `/api/v1/network/workspace-upsell`, `/api/v1/network/workspace-provision`, workspace inbox authorization callback |
| **Network Superconnector Reframe (Brief 270 parent + Briefs 271-279; Brief 278 now split into parent + sub-briefs 282-287)** | | | | |
| Parent Brief 270 — Reframe Ditto Network as an AI superconnector and split implementation into doctrine/IA, member signal, request, search, trust/privacy foundation, discovery/invites, watch, introductions, share loop, and trust/admin closeout | draft corrected by pre-build review (2026-05-14); pending fresh-context review + human approval. **Closeout gated by Brief 278 closeout (sub-brief 287) + human acceptance.** | User strategy steer 2026-05-14; extends Brief 254; review fix pass 2026-05-14 | Original to Ditto; Briefs 258-261 adopt existing KB/scout/profile/share/intro primitives; outcome-led pricing path from Brief 119 | `docs/briefs/270-network-superconnector-reframe-parent.md` |
| Brief 271 — Network doctrine, IA, copy, glossary, and `/network` home reframe around "superconnector for everyone" and concrete professional/economic outcomes | draft corrected (2026-05-14) | Brief 270 | Original to Ditto; `.impeccable.md` design doctrine; outcome-led copy from Brief 270 | `docs/briefs/271-network-doctrine-ia-copy-superconnector.md` |
| Brief 272 — Member Signal onboarding: LinkedIn/website/X/Instagram/user-provided sources, research, provenance, review, claim-token entry | complete (2026-05-15) — PASS with 2 non-blocking flags (pre-existing test baseline drift; orphan `0006_snapshot.json` from parallel Brief 273) | Briefs 270-271; Brief 258; Brief 256 | Brief 258 source-grounded KB/scout (adopt); Original claim provenance | `docs/briefs/complete/272-member-signal-onboarding-research-provenance.md`; `docs/research/272-member-signal-erd-note.md`; `packages/web/app/network/signal/page.tsx`; `packages/web/e2e/network-signal.spec.ts`; `src/engine/member-signal-*`; `packages/core/src/db/network/schema.ts` |
| Brief 273 — Need/Opportunity Request onboarding and manual search entry: need-first Active Request flow, outcome capture, calibration, mode choice | complete + approved (2026-05-18) — review blockers fixed; full baseline green | Briefs 270-271; Brief 257/264-266 | Existing `JobRequestCardBlock` flow (adopt); Original Active Request framing; in-place `networkJobRequests` evolution with legacy `open` compatibility | `docs/briefs/complete/273-need-request-onboarding-manual-search-entry.md`, `src/engine/need-request-*`, `/api/v1/network/requests`, `/network/request` |
| Brief 274 — Manual Search and Possible Connection proposals with why/evidence/risk/provenance/refine/save actions; no contact/discovery send | complete & approved (2026-05-18) | Briefs 272-273; Brief 258 | Brief 266 suggested candidates + Brief 258 scout (adopt); Possible Connection original | `docs/briefs/complete/274-manual-search-connection-proposals.md` |
| **Parent Brief 278 — Trust, Privacy, Admin, Observability for Superconnector Network** | **Architect design COMPLETE & fresh-context reviewed (APPROVE-WITH-MINOR-REVISIONS → all B1+B2+N1–N8 fixes applied) 2026-05-18; pending human approval.** Split per Insight-004 into foundation (sub-briefs 282/283/284) + closeout (sub-briefs 285/286/287). | Briefs 272-274 + 261 + parent 270; Designer `-ux.md` + Research-278 (10 resolved OQs incl. AgentMail Q5 RFC 8058 capability); Insight-238 (Curate 7th job, flagged for human ratify) | Brief 108 admin oversight + Brief 258 privacy + Brief 261 refusal controls + Brief 098b Svix-verified webhook pattern + AgentMail SDK v0.4.18 RFC 8058 headers / typed Complaint + FTC CAN-SPAM + GDPR/CCPA windows + LinkedIn/source-policy constraints; ADR-029 in-code platform-ToS precedent | `docs/briefs/278-trust-privacy-admin-observability.md`, `docs/insights/238-curate-is-the-seventh-human-job.md` |
| Sub-brief **282** — Network audit / scrubber / stop-run substrate (foundation start) | **implemented + fresh-context reviewed APPROVE after `/dev-review` fixes (2026-05-18); pending human approval**; reserves `prevHash` column for future hash-chain audit (R-Q2) | Parent 278 | `connection-proposal.ts` scrub precedent (follow, do not modify); `proposal-credential-scrubber.ts` (adopt-pattern); existing Network-tier append-only audit pattern (adopt); `createNetworkLaneStepRun` JSONL log as server-minted wrapper-run provenance | `docs/briefs/282-network-audit-scrubber-stoprun-substrate.md`; `src/engine/network-privacy-scrubber.ts`, `src/engine/network-audit.ts`, `src/engine/network-step-run.ts`, `src/engine/generate-share-variants.ts`; `packages/core/src/db/network/schema.ts`; `drizzle/network/0009_daffy_sir_ram.sql` |
| Sub-brief **283** — Source-policy enforcement / suppression / email compliance + complaint webhook (foundation middle) | **IMPLEMENTED & fresh-context reviewed APPROVE 2026-05-18; follow-up review-fix patch applied.** First review REVISE fixed: complaint circuit-breaker now counts per-event audit rows after Svix dedup, not suppression-state rows. Follow-up fixes: dedup claim + complaint writes are transactional, and suppression global uniqueness is enforced with partial unique indexes despite nullable `scope_user_id`. | Sub-brief 282 | `discovery-source-policy.ts` (in-code precedent per ADR-029); AgentMail `channel.ts AgentMailAdapter` extend (Researcher Q5); Svix verification pattern from `/api/v1/network/inbound/route.ts` (adopt); complaint-pause event counter original | `docs/briefs/283-network-source-policy-suppression-email-compliance.md`; `drizzle/network/0010_slimy_jubilee.sql`; `packages/web/app/api/v1/network/complaints/route.ts`; `src/engine/network-complaint-handler.ts`; `src/engine/network-suppression.ts`; `src/engine/network-email-compliance.ts`; `src/engine/discovery-source-policy.ts` |
| Sub-brief **284** — Privacy export/delete + retention + admin scaffold (**completes foundation checkpoint → unlocks 279**) | **implemented & fresh-context reviewed APPROVE 2026-05-19; pending human approval**; hybrid soft-delete-then-purge with anti-resurrection 410 tombstones; email-challenge DoS outer limiter now wired by 286 | Sub-briefs 282 + 283 | Insight-234 cross-deployment durability (adopt); existing `requireAdmin` + deployment-mode-gate (adopt — no second admin-auth system); claim-token identity (extend); retention windows from research (Architect-proposed defaults pending human ratification) | `docs/briefs/284-network-privacy-export-delete-retention-admin-scaffold.md`; new `/api/v1/network/privacy/{export,delete}` (email-challenge gated), retention scheduler, admin scaffold |
| Sub-brief **285** — Network Privacy Center (member UI; Curate's first full realisation) | **implemented & fresh-context reviewed PASS 2026-05-19; Designer refinement/elevation pass complete; Builder design-recommendation polish implemented; polish review PASS with flags recorded; pending human approval**; standalone `/network/privacy`, verified-identity fail-closed route boundary, chat drill-down, verified Discovery Profile four equal exits, guarded source/watch/block/profile/claim controls, provenance-first ledger UI with per-section rail status; **closeout member-facing surface**. Follow-ups before PR/handoff: include untracked Brief-285 files and pin rail-state labels in tests. | Sub-briefs 282-284, 286 | Existing ContentBlock primitives (`RecordBlock`+`KnowledgeCitationBlock`+`ActionBlock`+`StatusCardBlock`); Insight-238 ruling (no new ContentBlock); Brief 261 Hard Rule #5 (origin: Brief 259 system prompt); Original-to-Ditto Discovery Profile self-service | `docs/briefs/285-network-privacy-center-member-ui.md`; `packages/web/app/network/privacy/page.tsx`; `packages/web/components/network/privacy-center.tsx` |
| Sub-brief **286** — Admin health dashboard + dry-run replay + rate-limit (closeout admin surface) | **implemented & fresh-context reviewed APPROVE 2026-05-19; post-implementation `/dev-reviewer` fix-all re-reviewed APPROVE; pending human approval**; fixed-window L1+Postgres rate-limit, audited admin-reveal as the single documented intentional scrubber bypass, current route integrations complete, workspace-mode admin API hard-404, global-pause send guards, fail-closed pause UI, true action totals, structured reason validation, and broader anti-persona metadata scrub; watch-feedback producer remains Brief-275-bound | Sub-briefs 282-284 | Existing health/admin pattern; `visitor-rate-limit.ts` (extend); dry-run replay original; rate-limit algorithm pinned to fixed-window | `docs/briefs/286-network-admin-health-dashboard-dryrun-ratelimit.md`; `src/engine/network-abuse-controls.ts`; `packages/web/components/admin/network-health-dashboard.tsx` |
| Sub-brief **287** — Closeout: review-checklist gates + dictionary + regression + parent-270 acceptance (**closeout checkpoint → closes parent 270 after human acceptance**) | draft (2026-05-18); pending 285 + 286 | All 282-286 + 275-279 | 7 new Network trust gates enumerated (Member Signal provenance / private-leak scrub / no-contact background watch / two-sided intro consent / claim-before-public discovery / outbound-email compliance / source-policy enforcement-before-store-and-outreach) | `docs/briefs/287-network-closeout-review-gates-regression-acceptance.md`; `docs/review-checklist.md` (append-only); `docs/dictionary.md` (idempotent vs Brief 271) |
| Brief 279 — Outbound Discovery and Claim Invites: public/source-registry discovery, LinkedIn pointer-only posture, Discovery Profiles, superconnector scoring, operator-reviewed claim invites | **implemented & fresh-context reviewed APPROVE 2026-05-19; pending human approval**. Adds network discovery schema migration idx 13, guarded discovery/compose/send tools, admin operator queue, claim-token route/page, claim-token identity verification, and Discovery Profile/source/token retention + privacy export/delete wiring. Review fixes closed LinkedIn pointer-only storage, email-bound redemption, claim-token review, source-row export/delete/purge, durable send-attempt audit/revocation before external email, and prompt/tool parity. | Briefs 272 + 274 + 278 foundation (sub-briefs 282-284) | Brief 258 scout (adopt); LinkedIn official API/User Agreement constraints (pattern); Discovery Profile original | `docs/briefs/279-outbound-discovery-claim-invites.md`; `src/engine/public-people-discovery.ts`; `src/engine/claim-invite.ts`; `/api/v1/network/discovery`; `/api/v1/network/invites/[token]/claim`; `/admin/network/discovery` |
| Brief 275 — Background Watch and Network Health: request/member watches, discovery-candidate handoff, proposal queue, quiet digest, throttles | draft corrected (2026-05-14) | Briefs 273-274 + 278 foundation (sub-briefs 282-284) + 279 | Operating Cycle Archetype (adopt); Network health original | `docs/briefs/275-background-watch-network-health.md` |
| Brief 276 — Email/chat consent and introduction facilitation: two-sided approval, off-platform email thread, natural reply and outcome feedback | design complete, split per Insight-004 into Sub-briefs 288/289 (build 288→289→close parent 276); **Sub-brief 288 (consent state machine + `IntroProposalCardBlock` + decision emails + chat surface + approval route) IMPLEMENTED & remediated — prior fresh-context Dev Reviewer REJECT (B1 AC#17 consumer import / B2 AC#18 terminal-state propagation / B3 AC#19 e2e matrix / B4 AC#11 recipient scrub) all closed → fresh-context Dev Reviewer re-review APPROVE-WITH-NITS, 3 non-blocking notes closed in-scope (chat-surface recipient scrub / AC#16 delivery-linkage assertions / import-route idempotent re-ACK test), doc-drift #4–5 flagged for Architect (AC#1 "8-value" vs 7 outbound states; "What Changes" omits consumer-import surface); root+core+`pnpm run type-check` 0, Brief 288 targeted vitest 30/30; pending human disposition (2026-05-19)**; **Sub-brief 289 (reply ingestion + follow-up + outcome) IMPLEMENTED — schema, classifier, feedback fan-out, follow-up scheduler/email, feedback route, chat outcome surface; fresh-context review REVISE → blockers fixed → re-review APPROVE-WITH-NITS; direct live inbound seam regression added for the final nit; focused vitest 86/86 and `pnpm run type-check` 0 (2026-05-19)** | Briefs 275 + 261 + 259 + 278 foundation (sub-briefs 282-284) | AuthorizationRequestBlock and `notifyUser` (adopt); off-platform intro original; outcome feedback for later pricing | `docs/briefs/276-email-chat-consent-introductions.md`, `docs/briefs/complete/288-intro-consent-state-machine-and-decision-emails.md`, `docs/briefs/289-intro-reply-ingestion-followup-outcome.md` |
| Brief 277 — Share loop and public profile conversion: LinkedIn/X/Instagram/email/site share studio, public visitor conversion paths, outcome-led share moments | design complete, split into Sub-briefs 290/291/292 (build 290→291→292); **Sub-brief 290 COMPLETE (2026-05-19)** — fresh-context Dev Reviewer APPROVE, post-review fixes complete, gate re-green; **Sub-brief 291 COMPLETE (2026-05-19)** — signed `ditto_share_ref` cookie + host-bound `?ditto_ref` handoff, pure visitor-intent inference, always-clickable CTA strip, cookie-authenticated `network_share_attribution` route/table/audit, clean migration ownership at idx 16, focused vitest 122/122 + type-check 0; 292 draft (depends on 290's rate-limit/audit enum and now 291's visitor attribution surface) | Briefs 272 + 260 + 259 + 276 outcome feedback where available | Brief 260 share assets (adopt); in-house HMAC cookie helper (pattern); recognition-led and outcome-led sharing original | `docs/briefs/277-share-loop-public-profile-conversion.md`, `docs/briefs/complete/290-share-studio-channel-variants.md`, `docs/briefs/complete/291-attribution-visitor-conversion.md` |
...
| Operator field reconciliation (`greeter` + internal `system`; persona-guard collapse; Greeter mode coverage) | done | Brief 240, ADR-041, Insight-225 | Original to Ditto | `src/engine/cognitive-mode-resolver.ts`, `cognitive/modes/`, `processes/`, `docs/adrs/008-system-agents-and-process-templates.md` |
...
| **First Loop — Learn → Do → Loop spine (Briefs 247–251, ADR-046, Insight-222)** | | | | |
| Brief 247: parent — Greeter Beat 2 + recurring-process primitive + briefing delivery + surface trim | draft (decomposed) | Brief 247, Insight-222 | ADR-041 agency model, Brief 198 memory chokepoint | `docs/briefs/247-first-loop.md` |
| ADR-046: Loop primitive — generic `ownerKind`/`ownerId` + product-layer mapping + `LoopProductBridge` | proposed | ADR-046 | Original (mirrors `setSessionTrustResolver()` pattern) | `docs/adrs/046-loop-primitive.md` |
| Insight-222: Learn → Do → Loop spine (the principle) | active (in flight) | Insight-222 | Original | `docs/insights/222-*.md` |
| Sub-brief 248: Greeter Beat 2 authorization gate | builder complete (reviewed: pass with flags) | Brief 248 | `trust-gate.ts` handler pattern (adopt) + AI Elements Confirmation (adopt) | `docs/briefs/248-greeter-beat-2-authorization.md` |
| `authorization-request` ContentBlock (28→29) | builder complete (reviewed: pass with flags) | Brief 248 | Existing block-renderer pattern (adopt) | `packages/core/src/content-blocks.ts`, `packages/web/components/blocks/authorization-request-block.tsx` |
| Authorization gate handler (`packages/core/src/harness/handlers/authorization-gate.ts`) | builder complete (reviewed: pass with flags) | Brief 248 | `trust-gate.ts` (adopt) | `packages/core/src/harness/handlers/authorization-gate.ts`, `src/engine/heartbeat.ts` |
| Feedback-on-terminal-state (rejected/edit-requested/expired → memory chokepoint) | builder complete (reviewed: pass with flags) | Brief 248, Brief 198 | Brief 198 chokepoint (adopt) | core callback in `authorization-gate.ts`; chat path writes non-body decision metadata via `writeMemory()` |
| Gmail authorized-send tool + resolver registration | builder complete (reviewed: pass with flags) | Brief 248, Insight-180 | existing Google Workspace tool path (adopt) | `src/engine/tools/gmail-authorized-send.ts`, `src/engine/tool-resolver.ts` |
| Sub-brief 249: Loop creation primitive (`createLoopFromConversation()`) | draft | Brief 249, ADR-046 | `startProcessRun()` (adopt) | `docs/briefs/249-loop-creation-primitive.md` |
| `loop-confirmation` ContentBlock (29→30) | draft | Brief 249 | Existing block-renderer pattern (adopt) | `packages/core/src/content-blocks.ts` (planned) |
| `loop_owner_links` mapping table + 3 typed FKs + exactly-one-non-null CHECK | draft | Brief 249, ADR-046 | Original | `src/db/schema/network.ts` (planned) |
| `LoopProductBridge` interface + signature derivation (`SHA256(canonicalJson(...))`) | draft | Brief 249 | `setSessionTrustResolver()` pattern (adopt) | `packages/core/src/loops/` (planned) |
| Sub-brief 250: out-of-band briefing delivery (HMAC token + edit-at-desk) | draft | Brief 250 | `review-pages.ts` HMAC pattern (adopt), `notify-user.ts` chokepoint (adopt) | `docs/briefs/250-briefing-delivery.md` |
| `briefing_dispatches` table + composite INDEX `(schedule_id, dispatched_at DESC)` | draft | Brief 250 | Original | `src/db/schema/network.ts` (planned) |
| `BRIEFING_ACTION_VOCABULARY` const map (intent ↔ code action ↔ visitor copy) | draft | Brief 250 | Original | `src/engine/briefing-vocabulary.ts` (planned) |
| AgentMail integration spike (AC 31 — must pass before dispatcher wiring) | draft | Brief 250 | AgentMail SDK (depend) | `src/engine/briefing-dispatcher.ts` (planned) |
| Sub-brief 251: `/chat` surface trim + chat-session workspace-readiness gate | draft (Designer-gated) | Brief 251 | Brief 110/153 readiness pattern (adopt) | `docs/briefs/251-surface-trim-and-brief-153-trigger.md` |
| `/chat/auth?return=` allow-listed query-param thread (AC 4a) | draft | Brief 251 | Existing `/chat/auth` route (adapt) | `packages/web/app/chat/auth/route.ts` (planned) |
| Dual-trigger suppression — chat-session vs email-only path (AC 12a) | draft | Brief 251, Brief 110 | `checkWorkspaceReadiness()` (adopt) | `src/engine/workspace-readiness.ts` (planned) |
| Designer §3.4 (workspace-suggestion CTA spec) — pre-condition for builder | pending | UX spec at `docs/research/first-loop-ux.md` | — | `docs/research/first-loop-ux.md` |
...
**Status:** Brief 090 (Workspace Provisioning) complete. Brief 091 (Fleet Upgrades) complete. Brief 100 (Railway Migration) complete. Brief 153 (Workspace Provisioning Wiring) complete. Brief 267 complete. Phase 15 **complete**. All infrastructure now targets Railway (migrated from Fly.io), with provisioning health/env/login hardening added after the Launchpath boot incident.
...
| **Provisioner Auto-Env and Boot Hardening (Brief 267)** | | | | |
| Centralized managed workspace env + preflight | done (2026-05-12) | Brief 267, Insights 229/230 | Existing Railway provisioner saga | `src/engine/workspace-provisioner.ts` |
| Workspace-scoped bootstrap login | done (2026-05-12) | Brief 267, Brief 153 | Existing magic-link auth seam | `src/engine/magic-link.ts`, `packages/web/app/login/auth/route.ts`, `packages/web/middleware.ts` |
| Seed path-C sentinel and strict/provisioning health split | done (2026-05-12) | Brief 267, ADR-025 | Kubernetes readiness/provisioning probe split | `src/engine/network-seed.ts`, `packages/web/app/api/healthz/route.ts` |
| Workspace + Network schema health checks | done (2026-05-12) | Brief 267, Insight 229, ADR-048 | Drizzle journal comparison | `src/db/index.ts`, `src/db/network-db.ts` |
