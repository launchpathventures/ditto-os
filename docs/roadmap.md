...
| **Two-Sided Conversational Front Door (Brief 254 parent + Briefs 255-261 sub-briefs)** | | | | |
| `/network` parent brief (revision 2.1) — Ethos-pattern landing, two lanes, profile-as-chat, KB intake, share modal, intro authorization | spec-complete (reviewer-pass fixes applied) | Brief 254 | Ethos (pattern), process-os Ask Charlie (adapt with REPRESENTATIVE-not-IMPERSONATOR inversion), Brief 248 (extend) | `docs/briefs/254-network-two-sided-conversational-front-door.md` |
...
| Sub-brief 256 — Expert lane + new `NetworkProfileCardBlock` + `handle` field + 6-question directive + live-preview pane + handle-claim | pending; depends on 255 | Brief 254 unlock | Ethos v32 (pattern); engine boundary: `packages/core/src/content-blocks.ts` | TBD |
| Sub-brief 257 — Client lane + new `JobRequestCardBlock` (in `packages/core/`) + on-network candidate match + budget privacy enforcement | pending; depends on 255 | Brief 254 unlock | Original; budget-privacy is Soul Move | TBD |
