...
- **scopeType + scopeId**: Four memory scopes. Agent-scoped memories travel with the agent. Process-scoped memories stay with the project's processes (joined via `processes.projectId`); when ANY process in the same project runs, project-scoped memories load (Brief 227 — project memory scope discipline). Self-scoped memories (ADR-016, Brief 029) belong to the Conversational Self — user preferences, communication style, cross-session continuity. `scopeId` is the user identifier for self-scoped memories. Brief 227 adds an `appliedProjectIds: string[] | null` column on the `memories` table for the multi-project hybrid case: when `scopeType='self'` AND `appliedProjectIds` is non-empty, the memory applies only to the listed projects (a "promote to specific projects" hybrid scope); when `scopeType='self'` AND `appliedProjectIds` is null/empty, the memory applies everywhere (full self-scope). Person-scoped memories (Brief 079/080) store knowledge about a person in Ditto's relationship graph — contact context, interaction patterns, preferences. `scopeId` references `people.id`. Person memory isolation is via join path: `memories.scopeId` → `people.id` → `people.userId` (no `userId` column on the memories table). Person memories carry a `shared` flag (ADR-025, default `false`): when `true`, the memory is house-level institutional knowledge ("Priya prefers email") visible to all users across the centralized Ditto Network; when `false`, it is private to the creating user. Provenance: Mem0's scope filtering (`mem0/memory/main.py`), extended for the Self and person scopes.
...
When the harness runs a step from a sibling process in the same project, the same project-scoped memories load via the `processes.projectId` join (Brief 227 — corrections taught while running the morning-brief process apply to the evening-brief process when they belong to the same project). Single-process scope is the default for new memory writes (the writing process's id becomes `scopeId`); cross-project promotion requires explicit user action via the `promote_memory_scope` Self tool (Insight-127: humans control memory writes; agents propose, the user decides). The `demote_memory_scope` tool reverses the promotion. Pre-project-era memories (where the source process's `projectId IS NULL`) continue loading across all projects for backwards compatibility — no automated guess at which project they belong to.
