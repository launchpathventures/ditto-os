...
- **Classification:** **DEPEND** (Adopted in Brief 217 as the second cloud runner kind `claude-managed-agent`). Promoted from PATTERN→DEPEND-eligible→DEPEND on 2026-04-27. The bundled `@anthropic-ai/sdk` does not yet expose a `beta.managedAgents.sessions.*` surface; the adapter at `src/adapters/claude-managed-agent.ts` uses raw `fetch` against the documented HTTP contract (consistent with Brief 216's routine adapter pattern). Polling-primary status (30s cadence) with a 7-row terminal-state heuristic; conservative on ambiguous idle (GitHub PR-merged remains the authoritative success signal). Known limitation: archive does not interrupt in-flight tool execution.
...
### GitHub Actions workflow_dispatch — docs.github.com/en/rest/actions/workflows (Added 2026-04-25, Adopted in Brief 218 2026-04-27)
...
- **Classification:** **PATTERN** — Brief 218 uses raw `fetch` against `https://api.github.com` rather than depending on `@octokit/rest`. Rationale: the brief assumed `@octokit/rest` was "already in stack via Ditto's existing GitHub integration" but verification showed neither `@octokit/rest` nor any `octokit` package is installed (Insight-213 — Architect must verify SDK surface exists; flagged for Architect, mirrors the Brief 217 Anthropic-SDK precedent). The four endpoints in use (`createWorkflowDispatch`, `getWorkflowRun`, `cancelWorkflowRun`, `listWorkflowRuns`, `listRepoWorkflows`) are simple JSON; SDK affordance was not load-bearing. If a future brief adopts Octokit (e.g., for pagination-heavy workflows or webhook signature verification helpers), classify upgraded to DEPEND.
- **Ditto usage:** `runner=github-action` kind (Brief 218). Long-running tasks (≤6h/job, 35d/workflow), custom env via Actions secrets, native deploy gate via Environments. Cleanest fit for `vercel deploy --prod` because Actions OIDC is what Vercel/Netlify already consume.
- **Adopted in Brief 218 (2026-04-27):** `src/adapters/github-action.ts` implements `RunnerAdapter`. Webhook events route via the kind-agnostic `cloud-runner-fallback.ts` (`workflow_run` correlated by run id, `check_run` infrastructure for Brief 219 gating). 60s polling backup catches missed deliveries (`pollCadenceMs['github-action']: 60_000` in `packages/core/src/runner/poll-cadences.ts`). The 9-conclusion mapping table (incl. `stale → cancelled` for superseded runs per Reviewer IMP-1) lives in `mapWorkflowRunToDispatchStatus()`. Three callback modes (webhook-only / in-workflow-secret / in-workflow) progressively expose log-masking trade-offs. Template workflow at `docs/runner-templates/dispatch-coding-work.yml` ships as documentation (Ditto does not auto-commit YAML to user repos).
- **Known constraints:** the run-ID-on-response behaviour (since 2026-02-19) is the dispatch's ID-capture path; the adapter falls back to `listWorkflowRuns?event=workflow_dispatch&per_page=5` when the response lacks `id`. The 60s polling cadence is the recovery floor — sub-60s GitHub-event flakiness is invisible. Plaintext ephemeral tokens in `inputs.callback_url` MAY appear in workflow logs (90-day retention); the `in-workflow-secret` mode (long-lived bearer via `secrets.DITTO_RUNNER_BEARER`) is the safer alternative for callback-heavy workflows.
