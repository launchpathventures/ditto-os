...
function buildInternalCallbackSection(input: InternalCallbackInput): string {
  return [
    ...
    "      \"responseBody\": { /* OPTIONAL — see below */ }",
    "    }'",
    "",
    "State semantics:",
    ...
    "",
    "Include `responseBody` (a JSON object) IF AND ONLY IF your task asks you to return",
    "structured output (e.g., the project retrofitter expects",
    "`{commitSha, actuallyChangedFiles, skippedFiles?}`). Read your task instructions",
    "above for the exact shape. Omit the field when your task does not require it.",
    "",
