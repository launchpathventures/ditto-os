 ...
 * Sits between review-pattern and trust-gate in the harness pipeline.
 *
 ...
 *
 * Conditions are of the form: "output contains 'some text'"
 * or just a plain string to match against.
 */
 ...
 * Stringifies the first output value (or all values joined).
 */
...
export const routingHandler: HarnessHandler = {
  ...
  canHandle(context: HarnessContext): boolean {
    // Only run if step has route_to or default_next defined
  ...
  async execute(context: HarnessContext): Promise<HarnessContext> {
    ...
    // Store routing decision on context for heartbeat to consume
